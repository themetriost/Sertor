...
- **E10-FEAT-002** (unificazione ambienti): l'MCP doveva girare dal `.venv` canonico del workspace,
  MAI da un secondo venv costruito a mano (il vecchio `.venv-core`), sorgente di divergenza silenziosa
  (uno stantio mentre l'altro veniva sincronizzato).
- **E15-FEAT-001** (fedeltà dogfood, 2026-07-03): il runtime dell'agente è stato spostato dal `.venv`
  *editable* del workspace al **runtime installato** `.sertor/` (progetto `uv` che installa `sertor-core`
  da `git=HEAD`), per eliminare l'ambiguità repo-source ↔ installato. Da qui `.mcp.json` invoca
  `uv run --project .sertor python -m sertor_mcp.server`.

Questo test fallisce se `.mcp.json` regredisce a un venv non canonico (il vecchio `.venv-core`) o al
`.venv` editable del workspace (l'ambiguità che E15 ha chiuso), invece del runtime installato `.sertor/`.
"""
