"""Test US4 — reranking opzionale come secondo stadio (FR-010..014, FR-028).

Con un reranker fake conforme alla porta (structural typing): niente extra, niente rete.
L'assenza dell'extra si simula forzando l'ImportError (robusto anche se `flashrank` è installato).
"""
...
class FakeReranker:
    """Cross-encoder finto: privilegia i testi che CONTENGONO la query (deterministico)."""
...
def _populated(emb: FakeEmbedder):
    ...
    texts = {
        ...
        "b.py#0": "bersaglio bersaglio bersaglio",   # il preferito del cross-encoder
...
def test_enabled_reranker_reorders_fused_pool_with_its_scores(caplog):
    ...
    assert hits[0].chunk_id == "b.py#0"                # ri-ordinato dal cross-encoder (FR-010)
    assert hits[0].score == 3.0                        # score = punteggio del reranker
    assert fake.seen_pool_sizes == [3]                 # pool troncato a rerank_pool (FR-014)
    events = [r for r in caplog.records if getattr(r, "operation", "") == "rerank"]
    assert events, "manca l'evento rerank (FR-028)"
    rec = events[-1]
...
def test_disabled_reranker_returns_pure_rrf_without_event(caplog):
    settings = Settings.load(env_file=None)            # rerank_enabled=False di default
    fake = FakeReranker()
    eng = _engine(settings, reranker=fake)
    with caplog.at_level(logging.INFO, logger="sertor_core"):
        hits = eng.query("bersaglio", k=2)
    assert hits and fake.seen_pool_sizes == []         # mai invocato (FR-013)
    assert not [r for r in caplog.records if getattr(r, "operation", "") == "rerank"]
    no_rerank = _engine(settings)                      # senza reranker: identico
...
def test_rerank_configured_without_extra_raises_actionable_error(tmp_path, monkeypatch):
    # Simula l'assenza dell'extra anche se installato: import di `flashrank` → ImportError.
    ...
    assert "sertor-core[rerank]" in str(exc.value)     # azionabile (FR-012), mai fallback muto


def test_engine_importable_without_extra(monkeypatch):
    # FR-011: il modulo del motore non importa flashrank; solo l'adapter, e solo pigramente.
    monkeypatch.setitem(sys.modules, "flashrank", None)
    import importlib

    import sertor_core.engines.hybrid as hybrid_module
    importlib.reload(hybrid_module)                    # nessun ImportError
