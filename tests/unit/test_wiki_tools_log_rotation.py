"""Test FEAT-008 — log mechanics: daily rotation, curated append, scan, migrate.

Offline F.I.R.S.T. (`tmp_path`, no network). Covers SC-001..SC-005 of spec 008-meccanica-log.
"""
...
# --- US1: curated append + implicit rotation -------------------------------------------------

def test_append_creates_daily_partition_with_body(tmp_path):
    # SC-001 (file for its date) + SC-002 (curated body not reformatted).
    p = _profile(tmp_path)
...
def test_append_idempotent_on_heading(tmp_path):
    # SC-004 + DA-5: identity = heading; re-append with different body → no-op.
    p = _profile(tmp_path)
...
def test_partition_index_lists_days_idempotent(tmp_path):
    ...
    before = idx
    assert update_log_index(p) is False  # already up to date → no-op
    assert p.log_index_path.read_text("utf-8") == before


# --- US2: scan anchors on the most recent partition (hook parity) ------------------------------

def test_scan_anchors_on_latest_partition(tmp_path):
    # SC-003: pending count is relative to the most recent partition, not a single file.
    p = _profile(tmp_path)
    ...
    res = scan(p)
    assert res.schema == "wiki.scan/1"  # contract unchanged
    assert res.pending == 1  # only fresh is newer than the most recent partition


def test_scan_no_partitions_means_everything_pending(tmp_path):
    p = _profile(tmp_path)  # no partitions created yet
    src = tmp_path / "src"
    src.mkdir(exist_ok=True)
...
# --- US3: migrate (retroactive split) ---------------------------------------------------------

def test_migrate_splits_by_date_preserving_order(tmp_path):
    # SC-005: one partition per distinct date; entries on the same date → same partition, in order.
    p = _profile(tmp_path)
    ...
    assert d7.index("Primo del 7") < d7.index("Secondo del 7")  # order preserved
...
def test_migrate_idempotent_and_non_destructive(tmp_path):
    ...
    assert p.log_path.is_file()  # monolithic file not deleted (non-destructive)


def test_collect_and_lint_exclude_log_partitions(tmp_path):
    # Log partitions are not pages: collect/lint must not enumerate or flag them
    # (regression from activation: example wikilinks in old entries → false 'broken').
    ...
    assert not any(r.startswith("log/") for r in rels)  # partitions + log-index excluded
