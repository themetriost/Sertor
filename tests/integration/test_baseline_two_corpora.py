"""Polish — host-agnosticity of the baseline engine on 2 distinct codebases (SC-001)."""
...
def test_two_distinct_codebases_indexed_and_isolated(sample_repo, tmp_path):
    ...
    e1.index(sample_repo)               # no code changes between the two (SC-001)
    e2.index(_second_repo(tmp_path))

    paths2 = {h.path for h in e2.query("helper", k=50)}
    assert paths2                       # has results
    assert all(p in {"pkg/util.py", "README.md"} for p in paths2)  # isolated from the first corpus
