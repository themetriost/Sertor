...
| `.mcp.json` (`sertor-rag` server via `uv run --project .sertor`) — scope `project` (default) | **host root** | install: additive merge (preserves other MCP servers; a present-but-different entry is reported, not overwritten) · upgrade: the `sertor-rag` entry is reconciled in place |
