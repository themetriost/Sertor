...
works fine: `sertor-rag search` returns correct results. On some clients the only visible sign is a
connection that closes immediately (`CONNECTION_CLOSED`) at the start of every session.

**Cause.** The MCP SDK released **2.0.0** on 2026-07-28, removing the `mcp.server.fastmcp` submodule
that older Sertor versions import at the top of the server module. Any host that resolved its
dependencies after that date pulled the new major and froze it in `.sertor/uv.lock`; the server
process then dies on import, before serving anything — hence "none" rather than "some". `doctor`
stays green because its `mcp` check asks whether the server is *registered* in `.mcp.json`, not
whether it *starts*.

**This is fixed in Sertor 0.4.2 and later**, and the fix is in the code rather than in a version
bound: the server now runs on **both** SDK lines — `mcp` 1.2 up to (excluding) 2.3 — so a host whose
lock already froze the new major is healed by updating Sertor, with **nothing to change on your
side**.

**Who is affected.** Any host whose `.sertor/uv.lock` was written between 2026-07-28 and the day it
updates to 0.4.2 or later. Note that this includes hosts installed **long before** that date: the
upgrade procedure runs `uv sync --project .sertor --upgrade`, which **re-resolves** and can therefore
replace a safe 1.x with the new major. A host pinned to a published version before 0.4.2 cannot fix
this by re-resolving — updating Sertor is the fix.
...
A `ModuleNotFoundError: No module named 'mcp.server.fastmcp'` confirms this exact case. On 0.4.2 and
later the same command prints `server imports OK` whichever SDK line your lock holds; if instead you
see a message naming the installed `mcp` version and the supported range, your environment has a
version outside that range — install one inside it.

**Fix.** Update Sertor to 0.4.2 or later, then reload your MCP client (restart Claude Code, or
`/mcp reload` on the Copilot CLI):

```powershell
uvx --refresh --from "git+https://github.com/themetriost/Sertor.git@v0.4.2#subdirectory=packages/sertor" sertor upgrade
uv run --project .sertor python -c "import sertor_mcp.server; print('server imports OK')"
```

**Do not pin the SDK yourself** in `.sertor/pyproject.toml`: that file is regenerated by the
installer, so a local patch is debt you pay twice — and on this defect it is no longer needed.
