...
| FEAT-008 | **Ciclo di vita dell'installer** — `upgrade` degli artefatti già installati su un ospite e `uninstall` (rimozione pulita), oltre al primo install | Oggi `sertor install` fa solo il primo install (install≠run); upgrade/uninstall rinviati da `specs/011`/`012`/`015` **senza una casa durevole** | **Could** | **decomposta (2026-06-17)** → [`lifecycle-installer/requirements.md`](lifecycle-installer/requirements.md) (31 REQ EARS + 4 NFR; 4 domande aperte Q1–Q4 da girare all'utente prima del design); il *wizard di configurazione* è già coperto da FEAT-003 |
