...
| FEAT-010 | **Ergonomia & portabilità dell'installer** — fallback **`pip`** (oltre `uv`), **avviso target non-Python**, ~~**hook Linux**~~ *(→ scorporato)*, **install multi-target** (Claude + Copilot in un colpo). + **reviewer «clean-code» attivo** come superficie di governance (ricorre 3× senza casa) | Installer robusto su ospiti eterogenei (OS/runtime/assistente) | **Could** | 🔄 **A-09 hook portabili ✅ CONSEGNATA** (8 hook host-facing portati in **Python portabile**, `uv run --no-project python`, no `pwsh`; merge `15b9197` + migrazione `69d527c`/#163 — vedi EXEC) → [`portabilita-hook-python/`](portabilita-hook-python/requirements.md). **Resta** solo il *resto* da decomporre: fallback **`pip`**, **avviso target non-Python**, **install multi-target**, reviewer clean-code |
...
| FEAT-016 | **Default model per gli agenti `speckit.*` su Copilot CLI** — estende la model-policy di FEAT-015 anche ai 9 prompt-file `speckit.*` (`speckit.specify/clarify/plan/tasks/analyze/implement/constitution/checklist/taskstoissues`) vendorati da spec-kit via `specify init --ai copilot` (`.github/prompts/speckit.*.prompt.md`). **Bloccata a monte da una spike:** la doc ufficiale GitHub **non conferma** che i prompt-file supportino il campo `model:` (a differenza dei custom-agent `.agent.md`); e Sertor **non scrive** il frontmatter di questi file (li deposita `specify init`) → servirebbe un meccanismo di patch post-`specify` o una via nativa. Policy proposta dall'utente per questi: GPT per task meccanici/analisi (`speckit.tasks`/`checklist`/`taskstoissues`→`gpt-5.4-mini`, `analyze`/`constitution`→`gpt-5.5`), sonnet per `specify`/`clarify`/`plan`/`implement`. | Coerenza della model-policy su **tutta** la suite agenti SDLC, non solo i 5 Sertor-authored | **Could** | 📋 da spike — **promossa da FEAT-015 (2026-07-01)**, rinvio reale dallo scope di `specs/083`. Primo passo: verificare il supporto `model:` sui prompt-file Copilot + fattibilità patch post-`specify init` |
