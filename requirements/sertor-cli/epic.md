...
| FEAT-002 | **Installazione selettiva delle capacità del core** (motori RAG + skill wiki) su un repo target | Portare il core su un progetto, a scelta, senza eseguirlo | **Must** | decomposta — taglio `installer` (`requirements/sertor-cli/installer/requirements.md`, 2026-06-11): backbone `sertor` + `sertor install wiki`; `install rag` e `install governance` dichiarati ma non implementati in questo taglio |
