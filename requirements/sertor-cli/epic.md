...
| FEAT-010 | **Ergonomia & portabilità dell'installer** — fallback **`pip`** (oltre `uv`), **avviso target non-Python**, ~~**hook Linux**~~ *(→ scorporato)*, **install multi-target** (Claude + Copilot in un colpo). + **reviewer «clean-code» attivo** come superficie di governance (ricorre 3× senza casa) | Installer robusto su ospiti eterogenei (OS/runtime/assistente) | **Could** | 🔄 **A-09 hook portabili ✅** (8 hook host-facing portati in **Python portabile**, no `pwsh`; merge `15b9197`+`69d527c`/#163) → [`portabilita-hook-python/`](portabilita-hook-python/requirements.md). **+ multi-target · avviso non-Python · guida uv-assente ✅ CONSEGNATI** (merge `eb1f7a3`/PR #179, 2026-07-13 — `--assistant` CSV/`all`, container disgiunti, deps una volta, report aggregato/fail-fast · `host_env.is_python_host` → nota advisory · messaggio uv onesto) → [`ergonomia-installer/`](ergonomia-installer/requirements.md), vedi EXEC. **Resta:** fallback **`pip` reale** (rinviato a **FEAT-006**/go-public: deps workspace non pubblicate) · **multi-target su `sertor-flow`** (follow-up, stesso seam) · **reviewer clean-code** (feature di governance a sé) |
...
| FEAT-012 | **Consolidamento Copilot CLI-only** — un solo target Copilot (la **CLI**): rimozione del target VS Code (`AssistantId.COPILOT`, `.vscode/mcp.json`, prompt-file come veicolo comandi), naming uniforme `copilot-cli` tra `sertor` e `sertor-flow`, skill `requirements` resa custom-agent su CLI | Eliminare il footgun VS Code↔CLI e l'incoerenza di naming emersi dalla verifica empirica; supporto nativo di un solo target | **Should** | ✅ **CONSEGNATA (2026-06-17, PR #76)** — `specs/052-copilot-cli-only/`: rimosso `AssistantId.COPILOT`, `--assistant copilot` → errore esplicito; naming `claude\|copilot-cli` su sertor+sertor-flow; `requirements` custom-agent; mapping `copilot-cli→--ai copilot` (un punto). 530 test, Constitution 11/11, `sertor-core` invariato, non-regressione Claude verde. Requisiti [`copilot-cli-only/requirements.md`](copilot-cli-only/requirements.md) |
| FEAT-013 | **Auto-update version check (avviso d'aggiornamento al SessionStart)** — a inizio sessione un hook host-facing verifica che la versione installata di Sertor sia l'**ultima** e, se è indietro, **avvisa l'utente** di aggiornare (l'utente decide se/quando; l'avviso punta a `sertor upgrade` / `uvx --refresh`). Copre **tutte e 3 le dimensioni** (rag + wiki via `sertor`, governance via `sertor-flow`) — `/VERSION` a root è la fonte unica. Gemello di **E10-FEAT-011** (hook freschezza): stesso confine **D↔N** — l'hook **segnala**, l'utente **agisce**; **mai auto-upgrade**. | Gli ospiti non restano silenziosamente su una Sertor stantia; l'aggiornamento diventa visibile senza nag invasivo (adozione/portabilità, Principio X) | **Should** | ✅ **DONE (2026-06-26, PR #113 merge `8d951cd`)** — SpecKit completo specify→plan→tasks→impl: due hook host-facing (`version-check.ps1` SessionEnd: GET cachata ~24h del `/VERSION` + confronto stamp → `.sertor/.version-check.json`; `version-check-start.ps1` SessionStart Claude / prompt statico Copilot: avvisa se behind) + stamp `.sertor/.sertor-version` a install-time + `RUNTIME_IGNORES`. Constitution 12/12, `sertor-core` invariato. → [`version-update-check/requirements.md`](version-update-check/requirements.md) (18 REQ EARS + NFR). **Decisioni di scope:** (1) **home = questa epica E2** (dominio install/lifecycle/versione, vicino a `sertor upgrade`/FEAT-010); (2) «newer» = **bump di `/VERSION`** su `master` (GET raw, semantica «c'è una nuova versione», low-noise — **non** il SHA di commit); (3) cadenza = **check cachato ~1/giorno**, esito in `.sertor/` (es. `.sertor/.version-check.json`), **non-fatale offline** (no rete/errore → skip silenzioso, exit 0); (4) **solo avviso**, mai auto-upgrade. Riusa il pattern hook `SessionStart` di FEAT-011 (Claude = script; Copilot CLI = prompt statico) + i comandi `sertor upgrade`/`uvx --refresh` esistenti. **Cross-ref:** E12 usabilità (il *messaggio* UX «sei indietro, aggiorna»), E10-FEAT-011 (macchina hook + stato in `.sertor/`). **Prossimo passo:** `/requirements` (decomposizione EARS) → `requirements/sertor-cli/<feature>/requirements.md` |

| FEAT-014 | **Disciplina di versioning / bump di `/VERSION`** — oggi `/VERSION` è **manuale** (bump tools scartati come YAGNI in FEAT-001) ed è fermo a **`0.1.0` dal 2026-06-17**, invariato per **273 commit** (incluse tutte le feature da allora). **Conseguenza (finding 2026-06-30):** l'avvisatore FEAT-013 confronta lo stamp installato col `/VERSION` su `master`: a `/VERSION` mai bumpato il verdetto è **sempre `up-to-date`** → **l'avvisatore non scatterà mai un `behind`** (idraulica corretta, ma **inerte in pratica**). Serve una **decisione di politica**: (a) bump manuale di `/VERSION` (SemVer) quando si consegna qualcosa che gli ospiti dovrebbero prendere via `uvx --refresh` (zero codice, va ricordato); oppure (b) bump **automatico** (es. al merge su `master`, o derivato dai conventional-commit) = feature a sé. Tensione col modello `git+url@master`: ogni merge è *di fatto* una nuova versione, ma `/VERSION` non lo riflette. | Rende **utile** l'avvisatore FEAT-013 (oggi predisposto ma muto); gli ospiti vedono davvero quando sono indietro | **Should** | ✅ **decisa (A-15, 2026-07-10): bump MANUALE a ogni release user-facing.** `/VERSION` = SemVer, bumpato a mano **solo** quando si pubblica una release che gli ospiti dovrebbero prendere (`uvx --refresh`) — **non** a ogni merge (il tracking per-merge è compito del dogfood via HEAD, E15). Riframa lo stato: `0.1.0` fermo **non è drift** ma «nessuna release esterna ancora» → l'avvisatore FEAT-013 è correttamente **dormiente-fino-alla-release**, non morto; diventa vivo alla prima release. Zero codice; il bump automatico (opzione scartata) = YAGNI finché non c'è una cadenza di release / ospiti esterni reali. Regola in `CLAUDE.md` §Git & versionamento |
| FEAT-015 | **Default model-policy per i subagent Copilot CLI** — quando l'installer deposita la suite agenti per Copilot CLI, configurare anche una **mappa modello→agente di default** nel blocco `subagents.agents.<name>.model` del profilo Copilot CLI, invece di lasciare la selezione implicita. Riduce la variabilità tra installazioni, ottimizza costo/latenza sui task meccanici e tiene alta la qualità sulle fasi critiche. Policy proposta (default ragionato, non rigido): haiku per dispatcher/git (concierge, configuration-manager), sonnet per requisiti/design/implement/wiki, GPT per task meccanici/analisi (speckit.tasks/checklist/taskstoissues/analyze/constitution). **Requisiti:** solo Copilot CLI · merge non distruttivo · non sovrascrivere un modello scelto dall'utente senza conferma · idempotente · model-ID **centralizzati in un profilo versionato** · fallback **fail-loud** su modello non disponibile nel tenant. **Cross-pacchetto** (`sertor`: concierge/wiki-curator · `sertor-flow`: requirements-analyst/configuration-manager/requirements/`speckit.*`). | Default ragionati di modello → meno variabilità, costo/latenza ottimizzati, qualità alta sulle fasi critiche (adozione/coerenza) | **Should** | ✅ **consegnata** — vedi EXEC (merge `4e30d00`/PR #135, 2026-07-01). Meccanismo reale: `model:` nel frontmatter `.agent.md` (5 agenti Sertor-authored); estensione agli `speckit.*` = FEAT-016. |
| FEAT-016 | **Default model per gli agenti `speckit.*` su Copilot CLI** — estende la model-policy di FEAT-015 anche ai 9 prompt-file `speckit.*` (`speckit.specify/clarify/plan/tasks/analyze/implement/constitution/checklist/taskstoissues`) vendorati da spec-kit via `specify init --ai copilot` (`.github/prompts/speckit.*.prompt.md`). Policy proposta dall'utente: GPT per task meccanici/analisi (`speckit.tasks`/`checklist`/`taskstoissues`→`gpt-5.4-mini`, `analyze`/`constitution`→`gpt-5.5`), sonnet per `specify`/`clarify`/`plan`/`implement`. | Coerenza della model-policy su **tutta** la suite agenti SDLC, non solo i 5 Sertor-authored | **Could** | ❌ **WON'T / non-applicabile — spike 2026-07-13 (E2-FEAT-016).** La **doc ufficiale della Copilot CLI** ([custom instructions](https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-custom-instructions)) **non menziona i prompt-file**: la CLI legge solo `copilot-instructions.md`/`.instructions.md`(`applyTo`)/`AGENTS.md` + istruzioni locali. Gli `speckit.*` sono `.prompt.md` (costrutto **VS Code Chat**, non CLI-invocabile — conferma FEAT-011/012) → sulla CLI **nessuna superficie li consuma**, quindi **nessun meccanismo per `model:`**. In più [Issue #1195 `github/copilot-cli`](https://github.com/github/copilot-cli/issues/1195): `model` sui custom-agent ha avuto una regressione (dominio FEAT-015, non i prompt-file). Sertor è **copilot-cli-only** (FEAT-012): i prompt-file vivono in VS Code, **fuori scope**. La model-policy CLI utile (5 agenti Sertor-authored `.agent.md`) è già in **FEAT-015**. Riaprire solo se la CLI aggiunge il consumo dei prompt-file. |
| FEAT-017 | **Auto-updater: onestà su `unknown`/unreachable + go-public gating** — l'avvisatore d'aggiornamento (FEAT-013) è **strutturalmente inerte in produzione**: la GET di `/VERSION` da `raw.githubusercontent` dà **404** perché il repo è **privato** (verificato 2026-07-10) → `latest=""` → `verdict:"unknown"` permanente. La **macchina è sana** — validata **end-to-end dal vivo** (override `SERTOR_VERSION_CHECK_URL` https → fetch reale `9.9.9` → compare vs `0.1.0` → `verdict:"behind"` → l'hook SessionStart emette l'avviso `SERTOR UPDATE AVAILABLE`). Due nodi, **entrambi sciolti al go-public** (E2-FEAT-006, oggi Won't): (a) repo pubblico → fetch 200; (b) `/VERSION` bumpato (A-15). **Onestà (Principio XII):** oggi un `unknown` permanente è **invisibile** (il SessionStart avvisa solo su `behind`) → l'ospite non sa mai se l'updater è vivo; varrebbe un cenno **una-tantum** «impossibile verificare gli aggiornamenti» (senza nag). | L'ospite sa se l'updater funziona; nessuna capacità che finge in silenzio | **Could** | 🔄 **fetta onestà `unknown` ✅ CONSEGNATA (merge `00dcd62`/PR #180, 2026-07-13)** — SessionStart emette un cenno **una-tantum** «UPDATE CHECK UNAVAILABLE» quando `verdict=unknown` (GET fallita/repo privato), con flag persistente `unknown_notified` (no nag; si resetta a verdetto risolto); parità Copilot via prompt statico. Prima l'updater era **muto** su `unknown` (SessionStart parlava solo su `behind`) → l'ospite non sapeva che non poteva verificare. `sertor-core` invariato, +4 test parity. **Resta gated sul go-public** il pezzo strutturale (repo pubblico → fetch 200 invece di 404, + `/VERSION` bumpato A-15) = **FEAT-006**. *(finding 2026-07-10, macchina validata live.)* |
