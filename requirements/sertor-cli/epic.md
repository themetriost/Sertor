...
| FEAT-014 | **Disciplina di versioning / bump di `/VERSION`** — oggi `/VERSION` è **manuale** (bump tools scartati come YAGNI in FEAT-001) ed è fermo a **`0.1.0` dal 2026-06-17**, invariato per **273 commit** (incluse tutte le feature da allora). **Conseguenza (finding 2026-06-30):** l'avvisatore FEAT-013 confronta lo stamp installato col `/VERSION` su `master`: a `/VERSION` mai bumpato il verdetto è **sempre `up-to-date`** → **l'avvisatore non scatterà mai un `behind`** (idraulica corretta, ma **inerte in pratica**). Serve una **decisione di politica**: (a) bump manuale di `/VERSION` (SemVer) quando si consegna qualcosa che gli ospiti dovrebbero prendere via `uvx --refresh` (zero codice, va ricordato); oppure (b) bump **automatico** (es. al merge su `master`, o derivato dai conventional-commit) = feature a sé. Tensione col modello `git+url@master`: ogni merge è *di fatto* una nuova versione, ma `/VERSION` non lo riflette. | Rende **utile** l'avvisatore FEAT-013 (oggi predisposto ma muto); gli ospiti vedono davvero quando sono indietro | **Should** | 📋 da decomporre — **finding 2026-06-30** (verificato: `/VERSION`=`0.1.0`, ultimo touch `7f1c89f` 2026-06-17, +273 commit). Decisione aperta: bump manuale vs automatico |
| FEAT-015 | **Default model-policy per i subagent Copilot CLI** — quando l'installer deposita la suite agenti per Copilot CLI, configurare anche una **mappa modello→agente di default** nel blocco `subagents.agents.<name>.model` del profilo Copilot CLI, invece di lasciare la selezione implicita. Riduce la variabilità tra installazioni, ottimizza costo/latenza sui task meccanici e tiene alta la qualità sulle fasi critiche. Policy proposta (default ragionato, non rigido): haiku per dispatcher/git (concierge, configuration-manager), sonnet per requisiti/design/implement/wiki, GPT per task meccanici/analisi (speckit.tasks/checklist/taskstoissues/analyze/constitution). **Requisiti:** solo Copilot CLI · merge non distruttivo · non sovrascrivere un modello scelto dall'utente senza conferma · idempotente · model-ID **centralizzati in un profilo versionato** · fallback **fail-loud** su modello non disponibile nel tenant. **Cross-pacchetto** (`sertor`: concierge/wiki-curator · `sertor-flow`: requirements-analyst/configuration-manager/requirements/`speckit.*`). | Default ragionati di modello → meno variabilità, costo/latenza ottimizzati, qualità alta sulle fasi critiche (adozione/coerenza) | **Should** | 📋 **decomposta (2026-07-01)** — requisiti in [`default-model-policy-copilot/requirements.md`](default-model-policy-copilot/requirements.md) (16 REQ EARS + 8 NFR). **Finding critico verificato contro la doc ufficiale GitHub:** il meccanismo reale NON è `subagents.agents.<name>.model` (proposta utente inesistente) ma il campo **`model:` nel frontmatter YAML del file `.agent.md`** di ciascun custom-agent; i requisiti sono stati riscritti su questo meccanismo. Ambito ristretto ai **5 agenti Sertor-authored** (`concierge`/`wiki-curator` in `sertor` · `requirements-analyst`/`configuration-manager`/`requirements` in `sertor-flow`); gli agenti `speckit.*` (prompt-file vendorati da spec-kit) sono **fuori ambito dichiarato** (follow-up, non supportano `model:` per doc ufficiale). Riconcilia — senza contraddirla — la regola "`model:` sempre assente" di FEAT-011/049 (quella vietava il leak dell'alias Claude ereditato, non un `model:` Copilot deliberato). **Meccanismo A ratificato** (`model:` nel frontmatter, per-repo; scartato B settings machine-global). **SpecKit avanzato:** spec `specs/083-default-model-policy-copilot/spec.md` (checklist 18/18) + plan (Constitution **12/12 + Missione**, zero deroghe). **DA risolte:** DA-1 (override `/subagents` persiste in `~/.copilot/settings.json`, fuori dal repo → al sicuro per costruzione) · DA-2 (installer offline → nessun probe tenant; fail-loud disponibilità = documentale) · DA-3 (profilo versionato in `sertor-install-kit`) · DA-4/DA-5 chiuse nel plan (riconciliazione guardie via parsing valore `model:` non-substring — `claude-haiku-4.5` contiene `haiku`; nessun fallback strutturale, YAGNI). **Prossimo:** `tasks` → `implement` |
