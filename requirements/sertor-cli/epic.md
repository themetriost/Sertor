...
| FEAT-003 | **Configurazione** (provider LLM obbligatorio default cloud + Ollama; vector DB condizionale a scelta Chroma vs PGVector/MongoDB Azure) | Adatta le capacità all'ambiente target senza toccare codice | **Should** | lettura config ✅ in `esecuzione`. **Wizard decomposto + Q1–Q4 risolte (2026-06-17)** → [`configurazione/requirements.md`](configurazione/requirements.md) (32 REQ EARS; `sertor configure` separato, ibrido CI-safe, solo Azure/Ollama, validazione statica + `--check`). Causa-radice UX del RAG non configurato. **Pronta per `/speckit-specify`** |
