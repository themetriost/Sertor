...
| FEAT-003 | **SpecAudit** — confronto requisiti originali (`requirements/`) ↔ requisiti generati da SpecLift ("drift #2" nell'handoff): rileva quando i requisiti canonici del progetto sono andati alla deriva rispetto a ciò che il codice fa davvero | Chiude il loop lint-semantico con uno strumento dedicato invece del confronto manuale | **Could** | non decomposta — capacità futura, elencata per non perderla (handoff §7) |
