...
| FEAT-003 | **SpecAudit** — verdetto **per requisito** (SODDISFATTO / PARZIALE / MANCANTE / DRIFTED / NON_DOCUMENTATO, con confidenza e **citazione dell'àncora SpecLift**) confrontando i requisiti originali (`requirements/`) con l'output ancorato di SpecLift per lo stesso changeset. Back-translation top-down del gate pre-merge; **non legge mai codice/test/CI**, moat **strutturale** (completezza/integrità dei riferimenti/citazione senza riverifica, fail-loud) | Chiude il loop lint-semantico con uno strumento dedicato invece del confronto manuale | **Should** (era Could) | 🆕 **handoff ricevuto (2026-07-02)** — costruito e validato in Sinthari (`master`, PR #8, merge `91c5a45`; runtime `src/specaudit/`, skill host-agnostica, 58 test). Gemello di FEAT-001: **da vendorare in `sertor-flow`** (o `packages/specaudit`, decisione *stampo* aperta come FEAT-002). Consuma `*.speclift.json` → prerequisito = SpecLift self-hosted (FEAT-001 ✅, ora sblocca il loro T048/T049). Handoff archiviato in `wiki/sources/input-other-agents/processed/`. **Da decomporre** (`/requirements` → specify) |
