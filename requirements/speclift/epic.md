...
| FEAT-002 | **Distribuzione via installer** — rendere SpecLift/SpecAudit installabili su un ospite esterno che installa Sertor (skill + CLI, non solo testo) | Un ospite riceve SpecLift/SpecAudit funzionanti (skill + CLI) | **Should** | 🔄 **CASA CAMBIATA (2026-07-31, decisione D3) — NON è più nostra: è di *Sinthari*.** ⚠️ *La decisione precedente (2026-07-14: «fold in `sertor-flow`») è **superata**; era rimasta scritta qui fino al 2026-08-05.* **SpecLift/SpecAudit appartengono a Sinthari**, che si dichiarerà nodo e li distribuirà: **noi smettiamo di vendorarli**, e Sulcimen **non** li eredita (−3.916 righe e −52 test rispetto alla prima stesura del piano di separazione). Di conseguenza questa feature **non si implementa più come distribuzione nostra**: ciò che resta a noi è la **rimozione del vendoring**, tracciata come **E17-FEAT-008** (nodo Sulcimen) nell'epica [`separazione-ecosistema`](../separazione-ecosistema/epic.md), che chiude questa riga. **Provvisorio dichiarato:** SpecLift/SpecAudit entreranno in un nodo dedicato più avanti (domanda aperta §9.6 di E17). Gemella per SpecAudit (FEAT-003). |
...
- **[RISOLTA 2026-07-14]** Casa di distribuzione di SpecLift/SpecAudit → **`sertor-flow` (fold nel
  pacchetto)**, decisione utente. La tensione temuta («prerequisito RAG che romperebbe l'ortogonalità di
  sertor-flow») **NON si applica**: `speclift`/`specaudit` sono **zero-deps** e consumano il RAG **via
  MCP** (Adapter B), **senza importare `sertor-core`** — esattamente come sertor-flow. Scartati: pacchetto
  `sertor` (SpecLift è SDLC, non retrieval) e standalone (terzo installer da mantenere; e pubblicherebbe a
  nostro nome una capacità vendorata). Vedi riga FEAT-002.
