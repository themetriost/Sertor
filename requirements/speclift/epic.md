...
| FEAT-002 | **Distribuzione via installer** — rendere SpecLift/SpecAudit installabili su un ospite esterno che installa Sertor (skill + CLI, non solo testo) | Un ospite riceve SpecLift/SpecAudit funzionanti (skill + CLI) | **Should** | 🔄 **CASA DECISA (2026-07-14): `sertor-flow` — fold nel pacchetto.** `speclift`/`specaudit` (**zero-deps**, RAG via MCP → **non importano `sertor-core`**, quindi l'ortogonalità di sertor-flow al RAG **NON si rompe** — la tensione della vecchia §9 non si applica) diventano moduli + console-script di `sertor-flow`, **non** pacchetti PyPI separati (evita di pubblicare a nostro nome una capacità vendorata handoff). **Esclusi dal publish PyPI del go-public** (restano workspace-only finché la feature non è implementata). Implementazione = feature **media** (fold ~3900 righe zero-conflitto + distribuzione skill dual-target Claude/Copilot via `sertor-flow install` + test/guardie/doc) → da decomporre specify/plan. Gemella per SpecAudit (FEAT-003). |
