...
| FEAT-003 | **Skill: creare/indicizzare l'LLM Wiki** (indicizza il progetto in MD, documenta in continuo, archivia/distilla conversazioni) | Conoscenza persistente e cumulativa del progetto | **Must** | ✅ in produzione · **consolidata in FEAT-010** ([storico](wiki-creazione/requirements.md)) |
...
| FEAT-007 | **Skill: mantenere il wiki vivo** (spider/lint: rigenera indice, valida link, rileva orfani/contraddizioni, distilla raw→concept) | Tiene il wiki coerente e aggiornato (idempotente) | **Should** | **assorbita in FEAT-010** (manutenzione: lint + verifica di freschezza) |
...
| FEAT-010 | **LLM Wiki end-to-end** (consolidante, pattern Karpathy): generazione agentica al commit + on-demand + periodico; convenzioni `manual_edited`/`ingested_sources`; manutenzione (lint + verifica di freschezza) con gate human-in-the-loop; superfici skill/CLI/MCP; setup `sertor wiki init`. **Consolida FEAT-003 + assorbe FEAT-007**; vale anche su progetti senza codice | Capacità wiki completa e2e, production-grade, repo-agnostica | **Must** | [decomposta](llm-wiki/requirements.md) |
