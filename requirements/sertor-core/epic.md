...
|----|---------|--------------------|-------------------|-------|
| FEAT-001 | **Nucleo di retrieval condiviso** (ingestione repo-agnostica, chunking code-aware, embeddings multi-provider, astrazione vector store, facade di retrieval) | Fondazione production-grade su cui poggiano tutti i motori | **Must** | ✅ **DONE** — nucleo su `master` ([requisiti](nucleo-retrieval/requirements.md)) |
| FEAT-002 | **Motore RAG vettoriale (baseline)** production-grade | La capacità minima di "creare un RAG" interrogabile | **Must** | ✅ **DONE** — baseline su `master` ([requisiti](rag-baseline/requirements.md)) |
