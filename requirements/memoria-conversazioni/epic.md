...
| FEAT-003 | **Aggancio alla distillazione del wiki** — `distill`/`record` possono attingere a una conversazione archiviata come fonte grezza | Chiude il loop lossy: la distillazione ha finalmente una **fonte** | **Should** | decomposta → [`aggancio-distillazione/requirements.md`](aggancio-distillazione/requirements.md) |
| FEAT-004 | **Ricerca semantica opzionale sull'archivio** — embedding dei transcript come corpus/tier del RAG, opt-in (privacy §7) | Memoria episodica *per significato*, non solo per parola | **Should** | decomposta → [`ricerca-semantica/requirements.md`](ricerca-semantica/requirements.md) |
