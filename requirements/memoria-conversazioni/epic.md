...
| FEAT-010 | **Parità MCP per la lettura della memoria** — esporre via server MCP i comandi di lettura dell'archivio (oggi solo CLI) | Memoria interrogabile nativamente dall'agente, non solo da terminale | **Could** | ✅ **CONSEGNATA** (merge `427cf8e`/PR #208, 2026-07-20 — vedi EXEC). 3 tool read-only nel server `sertor-rag`: **`memory_search`** (full-text FTS5), **`memory_list`**, **`memory_show`** — thin sopra gli stessi servizi core della CLI (`MemoryArchive.list_recent`/`get`, `EpisodicSearch.search`), gated da `SERTOR_MEMORY`: memoria spenta → `{"status":"disabled"}` esplicito. Scope allargato a **search** (non solo show/list) = cuore di «interrogabile nativamente»; **semantico via MCP rinviato** (doppio gate `SERTOR_MEMORY_SEMANTIC`) → follow-up. Provato LIVE su archivio reale (14MB) + 7 test; `sertor-core` invariato (solo `sertor_mcp`). SpecKit completo. *(era `specs/036`/leak audit 2026-06-16.)* |
| FEAT-011 | **BUG: cattura memoria silenziosamente vuota su path con spazi (`encode_project_path`) + fallimento silenzioso** — `encode_project_path` (`adapters/capture/claude_code.py:34`) sostituisce `:`/`\`/`/` con `-` ma **non lo spazio**, mentre Claude Code collassa anche gli spazi quando nomina la cartella in `~/.claude/projects` → su un progetto il cui path contiene uno spazio la cartella calcolata **non combacia** con quella reale → source assente → **archivia 0 sessioni in silenzio** (`memory_capture_source_absent` = solo WARNING; `archived=0 errors=0`, exit 0). | (1) allineare l'encoding al comportamento reale di Claude Code (spazi **e** punti; es. regex `[^A-Za-z0-9]→-`, con test regressione su drive-letter/UNC/trattini/punti); (2) **fail-loud** (più importante): con `SERTOR_MEMORY=true` e source assente → **finding visibile / exit ≠ 0**, non `errors=0` silenzioso — viola il *Fail Loud* (Principio XII) che Sertor **prescrive agli ospiti**. Verificare anche l'adapter `copilot-cli` (stesso pattern `_absent`). | **Must** | ✅ **CONSEGNATA (merge `5d30635`/PR #189, 2026-07-15)** — vedi EXEC. (1) `encode_project_path` → `re.sub(r"[^A-Za-z0-9]", "-", …)`, **validato dal vivo** (VM-WorkingFolder: 22 sessioni recuperate); (2) fail-loud host-agnostico: `source_available()` sul port + `ArchiveRunReport.source_absent` + WARNING nel formatter (human+JSON), exit 0 preservato. Test: 15 campioni reali + fail-loud; `copilot-cli` non affetto. `sertor-core` unico impatto. Requirements in `fix-cattura-encoding-e-fail-loud/`. **BUG confermato sul codice (verifica 2026-07-14), fonte: handoff Nunzio `memory-archive-silenzioso-path-con-spazi.md` (2026-07-09).** Impatto **misurato**: 3/13 progetti con spazi affetti (VM-WorkingFolder perde **22 sessioni**); Windows: path con spazi = norma. **Distinto dal 🐛 auto-capture E4** (Sertor no-spazi NON affetto: archive manuale = 58 sessioni, come predetto). Due fix indipendenti; il 2° avrebbe fatto emergere il bug il giorno stesso |
| FEAT-012 | **BUG: cattura-auto mai eseguita — gate privacy sull'ambiente sbagliato** — l'hook `memory-capture` (SessionEnd) gata su `os.environ["SERTOR_MEMORY"]`, ma il valore vive in `.sertor/.env` (letto solo da `Settings.load` dentro la CLI, non iniettato nell'ambiente hook) → gate sempre `None` → no-op silenzioso su ogni host che abilita la memoria via file → cattura mai eseguita. Regressione della migrazione A-09 (`.ps1`→`.py`). **Distinto da FEAT-011** (encoding) e **dall'ipotesi errata FEAT-031/032** (path relativo = *secondo* difetto stacked, risolto). | Allineare il gate dell'hook alla stessa fonte che la CLI osserva (il `.env` risolto), stdlib-only e host-agnostico; preservare il default privacy (spenta → no-op). | **Must** | ✅ **CONSEGNATA (merge `9921a34`/PR #210, 2026-07-21)** — vedi EXEC. `_hooklib.memory_enabled()` legge `./.env`→`.sertor/.env` (ancorato a `CLAUDE_PROJECT_DIR`, `.env` vince su `os.environ`), byte-copiato Claude/Copilot; +5 test + whitelist `SERTOR_MEMORY` nella guardia host-agnostica; provato LIVE. `sertor-core` invariato (solo asset hook). Requirements in `feat-012-fix-cattura-auto-gate/` |
| FEAT-013 | **Ricerca semantica della memoria via MCP** — il tool MCP `memory_search` accetta `semantic=true` (ricerca per significato), mirror di `sertor-rag memory search --semantic`, così l'agente fa recall semantico sul vehicle nativo senza subprocess. Chiude il residuo «semantico via MCP» rinviato da FEAT-010. | Portare anche la ricerca *per significato* sull'MCP (parità con la CLI), dietro il doppio gate `SERTOR_MEMORY` + `SERTOR_MEMORY_SEMANTIC`; disabled onesto che nomina il knob giusto, mai fallback silenzioso al full-text. | **Should** | ✅ **CONSEGNATA (merge `727ab5b`/PR #212, 2026-07-21)** — vedi EXEC. `memory_search(semantic=true)` thin sopra `build_memory_semantic_index` + `SemanticMemoryQuery`; hit shape condivisa col full-text, query hashata; +6 test (colta collisione `_hit_dict`↔SymbolHit). `sertor-core` invariato (solo `sertor_mcp`). Provato LIVE sul core path. Requirements in `feat-013-semantico-mcp/`. |
| FEAT-014 | **BUG: `memory search` fallisce (mascherato) su query con punteggiatura** — il testo grezzo va nel `MATCH` FTS5 senza quoting; un token tipo `0.1.1`/`a/b.py`/`tipo:esito` → `fts5: syntax error`, catturato e **degradato a `(no results)`** (masking, anti-Fail-Loud). Colpisce CLI + tool MCP `memory_search`. | Sanitizzare il testo libero (ogni token come string-literal FTS5) a monte di `_build_sql`, così la punteggiatura è contenuto; il masking sparisce per prevenzione. | **Should** | ✅ **CONSEGNATA (2026-07-21)** — funzione pura `_to_fts_match` (quoting per-token, AND preservato), `_build_sql` prende il match sanitizzato; help CLI corretto («free-text, punctuation-safe»). +7 test + **LIVE** (RAW `0.1.1` crasha → SANITIZED trova 13 match reali prima mascherati). `sertor-core` (solo `episodic_search.py` + help CLI). **Fonte: nodo Acta** (bacheca, verificato sul codice). Requirements in `feat-014-fts5-query-punteggiatura/`. |
| FEAT-015 | **Continuità di sessione: al riavvio, iniettare nel contesto gli ultimi turni di conversazione** *(richiesta utente 2026-07-29)* — oggi l'archivio episodico **conserva** tutto ma si consulta solo **a domanda** (`memory search`, o i tool MCP): riavviando, l'agente riparte senza la coda di ciò che si stava facendo, e la ricostruisce chiedendo o rileggendo. L'idea: al **SessionStart** iniettare gli **ultimi ~2 turni** della conversazione precedente, così la ripresa parte da dove si era interrotta. Il materiale **esiste già** (FEAT-001 cattura, FEAT-002 lettura): manca la **superficie di restituzione** all'avvio. Complementare all'iniezione già presente (indice wiki + EXEC + ultimo log), che dà lo *stato del progetto* e non il *filo del discorso*. | Il caso più frequente della memoria non è «ne avevamo già parlato mesi fa» ma «cosa stavamo facendo cinque minuti fa»: oggi è l'unico che l'archivio non serve | **⚠️ DA ELICITARE** | 📋 **da elicitare** — richiesta 2026-07-29, non ancora requisito. **Domande aperte da porre prima di qualunque design:** *quali* turni (solo `user`? coppie user+assistant? gli ultimi N o l'ultimo scambio completo?) · **quale sessione** (l'ultima **di questo progetto**? e se l'ultima è stata su un altro progetto o un altro assistente?) · **budget di contesto** (2 turni possono essere enormi: si tronca? si riassume — e allora è **giudizio**, quindi non nel core? — o si dichiara il troncamento?) · **privacy** (l'iniezione automatica è più invasiva della ricerca a domanda: eredita il gate `SERTOR_MEMORY` o ne vuole uno proprio, e resta **off di default**?) · **quando** (ogni SessionStart, o solo su `--continue`/riavvio ravvicinato? l'iniezione a freddo di una conversazione di tre settimane fa può **disorientare** invece di aiutare) · **confine D↔N** (il canale-hook è limitato a ~10.000 caratteri: si iniettano i turni, o si **istruisce** il flusso principale a caricarli con un tool, come già fa l'hook wiki?) · **rapporto con `remember-this`** (FEAT-005) e con la distillazione (FEAT-003), che lavorano sullo stesso archivio. |
