...
| FEAT-015 | **Visibilità del RAG nella TUI (dimostrabilità)** — mostrare nella TUI **query · operazione MCP · risultato · verdetto hit/miss** per le ricerche recenti; realizza l'**opt-in raw-text (REQ-E9)** per uso LOCALE (default privacy-safe preservato) | Vedere e **dimostrare** *come* funziona il RAG (NON audit/tracciamento) | **Should** | 🔄 **implementata (feature 064)** — opt-in `SERTOR_OBSERVABILITY_CONTENT` (default off, richiede lo store): eventi retrieval portano query+preview+snippet (scrubbati) + `abstained`; scheda TUI **"RAG"** con verdetto hit/miss/astenuto; verificata live. *MCP query-arg (REQ-006) = follow-up.* PR in apertura |
