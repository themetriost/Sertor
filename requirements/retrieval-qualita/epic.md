...
| FEAT-003 | **Qualità del retrieval fuso code+doc (`search_combined`) su query NL/architetturali** — alzare la pertinenza NL **per-superficie** (doc + code) con la **fusione** come test d'integrazione; metrica di **fusion coverage**; categoria dedicata ai casi **requisito→implementazione** | Il **differenziatore** della mission: codice (*cosa*) + documenti (*perché*) fusi per l'agente | **Should** | 🔄 **T1 misura + T2 contratto CONSEGNATI (2026-06-21, merge `42aceaf`+`908bd92`)** → [`qualita-search-code-nl/`](qualita-search-code-nl/requirements.md): T1 = set NL `intent`-typed + misura per-superficie + `eval run --fused`; T2 (feature 070) = **`search_combined` → tupla `(docs, code)`** + metrica **OR/unione** (`union_hit_rate`=1.0). **Scoperta empirica:** lo «0.17» era artefatto della metrica AND (corretta a OR, P2.b aggiornato); il vero debole è **`search_docs` MRR 0.55 *(numero di giugno 2026, superato: dopo il dedup fuzzy il pavimento registrato e' `search_docs` hit@5 0.88 / hit@3 0.625, gate PASS — e la superficie di prodotto e' comunque `search_combined` a tre flussi, non `search_docs` isolato)***. **`search_docs` affrontato ✅** (dedup near-duplicate, merge `67b4177`/#158, 2026-07-07: @5 0.75→0.88; il debole era duplicazione di contenuto CLAUDE.md↔bundle). **Resta la leva PROFONDA** (nuova, tracciata): competizione tra doc *correlati* (non-duplicati) al confine k=3 — contextual retrieval / re-ranking di diversità, **solo se lift misurato** (no LLM nel run, RNF-3) |
...
| FEAT-011 | **Valutazione della navigazione del grafo (set-based)** — misura la **correttezza delle query relazionali** del code-graph (`who_calls`, dipendenze/callees, `find_symbol`, `related_docs`): la risposta è un **insieme di simboli/nomi** (qualname), non un file in cima a un ranking. Oracolo a **insiemi** (precision/recall/F1 o match esatto), eseguito via la porta `CodeGraph` (vehicle, Principio XI), **senza** rank né @k. Possibile genesi a *snapshot* dal grafo corrente (approvato una volta → sentinella di regressione). *(Promossa 2026-06-20: l'harness `evaluate` hit@k su path NON può esprimere una navigazione relazionale; emerso provando live `eval-feedback` su una query "chi chiama / da cosa dipende".)* | Rende misurabile la potenza relazionale del grafo, oggi non valutata | **Should** | ✅ **DONE (2026-06-20)** → [`valutazione-navigazione-grafo/`](valutazione-navigazione-grafo/requirements.md) |
| FEAT-012 | **Contratto di retrieval verso l'agente (fan-out del grafo, misurato)** — porta il segnale **strutturale** all'agente dentro `search_combined` (terzo flusso etichettato, dietro interruttore) **senza che debba chiederlo**, e prima ancora costruisce l'**harness di valutazione agent-facing** che ne è il gate. Include: blindatura con test delle invarianti della valutazione strutturale (oggi vere ma non protette); dichiarazione dell'ambito di validità del punteggio **coerente col motore configurato**; **assenza tipizzata** (non tentato / vuoto legittimo / non tentabile, con 3 cause distinte) e troncamento dichiarato; provenienza degli ingressi; marcatura della corroborazione fra flussi. **Gate duplice vincolante:** beneficio sulle domande strutturali **e** non-regressione su quelle non strutturali. *(Nasce dalla design note [[llm-facing-retrieval-contract]], 4 giri di revisione esterna. Scoperta abilitante: la macchina di valutazione esistente misura il **retrieval**, non il **comportamento dell'agente** — le 4 affermazioni empiriche del contratto non erano misurabili con gli strumenti attuali.)* | Chiude il divario fra un grafo **costruito a ogni index e caricato a ogni avvio** ma raggiungibile solo se l'agente lo chiama, e l'evidenza che sulle domande strutturali la similarità è scarsa (hit@1 ≈ 0.18) dove il grafo è esatto (0.64). E dota il progetto della **prima misura agent-facing**, riusabile per ogni futura decisione di contratto | **Must** | ✅ **CONSEGNATA** — rilasciata in **v0.2.0** (2026-07-24): il code-graph e' il **terzo flusso** di `search_combined` sul vehicle MCP (`sertor_mcp/server.py`, chiave `graph` + `graph_status`), acceso di default. *La facade di libreria resta a due flussi per progetto — il terzo vive sulla superficie che l'agente consuma.* Requisiti: [`contratto-retrieval-agente/`](contratto-retrieval-agente/requirements.md) — vedi EXEC |
| FEAT-013 | **Harness di valutazione agent-facing** — misura l'effetto della **forma** di un risultato sulla **risposta dell'agente**, non sui risultati recuperati. Stessa domanda, due varianti di payload identiche tranne l'aspetto in esame, regola di decisione **registrata prima** dell'esecuzione, verdetti **deterministici** (cita le fonti attese? cita fonti assenti dal materiale?), N ripetizioni per distinguere l'effetto dal rumore, dimensione del payload e latenza registrate accanto alla qualità. *(Erano le fasi F7+F8 di FEAT-012, **promosse a feature propria il 2026-07-24**: sono una capacità di misura riusabile, non un dettaglio di consegna di quella feature — e FEAT-012 è stata consegnata **senza** attendere il gate, per decisione utente.)* | **Colma il divario scoperto scrivendo [[llm-facing-retrieval-contract]]:** la macchina di valutazione esistente misura il **retrieval** (hit@k/MRR su ciò che viene recuperato), non il **comportamento dell'agente** che quel materiale lo legge. Fra le due cose non c'è ponte, e senza ponte ogni decisione di contratto si prende a opinione. Serve a tutte le affermazioni empiriche aperte: se il punteggio nel payload sia utile (per motore), se l'ordine ancore-prima aiuti, quale serializzazione regga, se dichiarare l'ambito basti a impedire i confronti cross-flusso, e se il fan-out del grafo giovi senza diluire | **Should** | da decomporre — **richiede un agente e ha un costo per esecuzione** (~450 chiamate per il ciclo completo): non gira nella suite né in CI, per costruzione |
