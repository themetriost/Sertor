...
| FEAT-005 | **Rust nel corpus** — `.rs` non è in `_EXT_LANG` (`services/ingestion.py:21-36`, 16 estensioni): i file Rust non entrano né come codice né come testo. Serve la voce di mappa + il chunking tree-sitter, come per gli altri linguaggi già presenti. **La domanda a monte è più generale della risposta:** l'elenco è una **allowlist chiusa**, quindi ogni ospite scritto in un linguaggio non previsto è fuori dal corpus — aggiungere Rust chiude un caso, non la classe (il *vedere* l'esclusione è E10-FEAT-079). | Un nodo della federazione scritto in Rust può interrogare il proprio prodotto. Oggi **non può** | **Should** | 📋 da fare — richiesto dal nodo **Kaelen** (104 file `.rs` fuori dal corpus), bacheca Acta 2026-07-31, rimisurato il 2026-09-18 |
