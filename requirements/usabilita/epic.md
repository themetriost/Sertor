...
| FEAT-014 | **La prima query RAG di una sessione impiega >2 minuti, e nulla lo dice** — misurato sul nodo *Acta* (corpus 150 doc): prima `sertor-rag search` della sessione **>120 s** (oltre il timeout del loro tool), stessa query a caldo **2 s**. **Non è un guasto** — l'esito è corretto — è un problema di **percezione**: al primo colpo il comando sembra bloccato, e chi non aspetta due minuti conclude che il RAG è rotto e **passa a leggere i file a mano**, cioè esattamente il comportamento che l'istruzione «search first, read second» esiste per evitare. Rimedio minimo: **dirlo** — una riga su stderr («primo avvio: inizializzazione del provider, può richiedere un minuto») costa nulla e trasforma un sospetto di guasto in un'attesa consapevole. Prima va **misurata la causa** (warm-up provider/modello? caricamento indice? BM25?) con i tempi spezzati per fase: *Acta* si è offerto di misurarli sul proprio host. | Il primo contatto con la capacità decide se l'utente si fida: un'attesa muta di due minuti insegna a non usare il RAG | **Should** | 📋 da fare — segnalato dal nodo *Acta* (2026-07-24, punto 3) |
