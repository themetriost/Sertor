...
| FEAT-010 | **Discoverability & resolution dei CLI `sertor-*`** — gli asset **agent-facing** (blocco `SERTOR:RAG-USAGE`, skill `guided-setup`/`wiki-author`/eval, agente `concierge`, doc di setup) insegnano **come invocare** Sertor, a due livelli: **(a)** runtime `sertor-rag`/`sertor-wiki-tools` → forma **robusta** `uv run --directory .sertor <cli>` (stessa che usa l'MCP; comando nudo solo come fast-path se sul PATH; fallback al `.venv/{Scripts,bin}/<cli>`; **fail-loud azionabile** se nessuno risolve — mai fallback manuale silenzioso); **(b)** installer `sertor` → **effimero** via `uvx --from "git+…#subdirectory=packages/sertor" sertor <verb>` (NON un comando persistente). + **nota di compatibilità** Python 3.14/`pywin32` (rumore `pywin32_bootstrap` sul Python **di sistema**, non bug Sertor: i CLI girano nel `.venv` via `uv run`; non usare `pip show` di sistema per verificare). Guardia anti-regressione: i body degli asset NON devono contenere comandi `sertor-rag`/`sertor-wiki-tools` nudi. Opz.: shim su PATH in `sertor install`; check «CLI raggiungibili?» in `doctor`. | Chiude un falso «command not found» che **rompe guided-setup out-of-the-box su Windows** e blocca le operazioni wiki (CLI-only, nessun fallback MCP), violando il fail-loud | **Must** | 🔄 in corso (branch `usabilita-feat010-discoverability`) — 3 finding **confermati live** su host Windows reale (2026-06-23): (1) `sertor-rag` non sul PATH, (2) `sertor` installer effimero non documentato, (3) Python 3.14/pywin32 rumore. Stessa radice: gli asset non insegnano l'invocazione |
