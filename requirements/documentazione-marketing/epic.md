...
| FEAT-014 | **Guardia DETERMINISTICA contro il drift della documentazione utente** — oggi la doc utente (`docs/`, `README.md`, `CHANGELOG.md`) è presidiata **solo da disciplina** (regola 3 del `CLAUDE.md` + lint semantico livello B, entrambi **giudizio**): nulla di meccanico si accorge se un comando documentato non esiste più, se un flag citato è stato rimosso, se un comportamento dichiarato è stato invertito. **Un README che dichiara un comportamento che il codice non ha più è peggio di un README assente: è una bugia con l'autorevolezza della documentazione.** Il nodo *Acta* ce lo chiede avendo appena pagato il caso: **quattro derive in dieci giorni** su un progetto piccolo e attivo — README che promette una protezione (`install` non sovrascrive una skill modificata) **invertita in v0.3.1**, installazione `--editable` superata, `subscribe` e l'occasione non menzionate, `doctor` descritto con 4 check quando ne fa 5. E il nostro caso reale gemello: il comando d'aggiornamento pubblicato nelle release notes **falliva su ogni host** mentre `docs/` aveva la forma giusta (chiuso in v0.2.0/D2, ma **nessuna guardia** l'aveva colto — l'ha colto un ospite). Idee di meccanismo, tutte deterministiche e zero-LLM: estrarre i **comandi/flag citati** nella doc e verificarne l'esistenza contro la CLI reale (`--help`) · eseguire i comandi documentati in uno smoke di CI · un check `doctor`-style doc↔realtà. Confine D↔N: la guardia **trova**, il lint semantico (giudizio) **decide cosa riscrivere**. | Fase 2 | Chiude la **classe** che tre nodi hanno nominato indipendentemente — *l'artefatto dichiara uno stato, la realtà ne ha un altro, e niente confronta i due* — sull'ultimo fronte che ne è privo: installer e marker di versione avranno il loro fix, la doc no | **Should** | 📋 da fare — richiesta esplicita del nodo *Acta* (2026-07-24, `acta-domanda-struttura-documentazione-e-guardia-drift`, domanda 4 = «**la** domanda») |
| FEAT-015 | **Un emendamento si referenzia per TITOLO, non per numero** — l'annuncio della v0.3.0 diceva ai destinatari «il **Principio XIV** è lì da leggere», ma nell'artefatto che i destinatari leggono — lo starter 0.5.0 — quel principio è il **XIII**. Entrambi i valori sono corretti; il difetto è **aver citato la propria numerazione a un pubblico che vive in un'altra**. E la radice è **strutturale, non tipografica**: la numerazione dell'ospite **non può** coincidere con la nostra per costruzione, perché la sua costituzione è sua da estendere — è la stessa autonomia che rende buono il meccanismo. Quindi non si corregge rinumerando. Rimedi: convenzione scritta (annunci, CHANGELOG, `Amendments` citano **titolo** + versione dello starter, mai il numero d'ordine) + guardia che vieti `Principio [IVX]+` negli asset e nelle note destinate agli ospiti. *Segnalato dal nodo Acta, che nota **dove** cade: nell'annuncio del principio che vieta a un fatto di vivere in due posti senza riconciliatore. Il numero vive in due posti con valori diversi e corretti, e niente li collega.* | Fase 2 | Ciò che diciamo all'ospite dev'essere vero **nel suo** sistema di riferimento, non nel nostro | **Should (P1)** | 📋 da fare — dal nodo *Acta* (2026-07-26) |
