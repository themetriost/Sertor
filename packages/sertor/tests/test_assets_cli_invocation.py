...
*invokes* a CLI uses the cwd-independent form `uv run --directory .sertor <cli> …` and ships the
canonical "How to invoke" guidance.

This suite asserts, offline (no network, no `uv`):

  (a) **presence** of the robust form `uv run --directory .sertor` in the bodies that drive CLI use
  ...
  (b) **scoped absence** of bare *invocations* — a line that runs `sertor-rag`/`sertor-wiki-tools`
      followed by a subcommand, NOT preceded by `uv run --directory .sertor` (or `… `, the
...
_ROBUST = "uv run --directory .sertor"
...
# example `uv run --directory .sertor\nsertor-rag …` is seen as robust.
_CLI = r"sertor-rag|sertor-wiki-tools"
_EXEC_ARG = r"(?:\.\.?|--[a-z]|run\b|init\b|\"|<[a-z])"
_BARE_INVOCATION = re.compile(
    r"(?P<prefix>.{0,40}?)`?\b(?P<cli>" + _CLI + r")\s+(?P<sub>"
    + "|".join(_SUBCOMMANDS) + r")\s+" + _EXEC_ARG
)


def _is_robust_context(prefix: str) -> bool:
    """The invocation is robust if its immediate left context carries the venv-routing prefix."""
    return "uv run --directory .sertor" in prefix or prefix.rstrip().endswith("…")
...
def test_wiki_playbook_ships_runtime_invocation_guide():
    ...
    It must teach `uv run --directory .sertor` and the PATH-failure clarification, while staying
...
def test_rag_usage_block_uv_run_replaces_bare_search():
    """The RAG-USAGE block's `search` example is now routed through `uv run --directory .sertor`."""
...
def test_bare_invocation_regex_passes_robust_and_namecitation():
    ...
    robust = "run `uv run --directory .sertor sertor-rag index .` to build"
