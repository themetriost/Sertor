...
- **CLI** — run `uv run --directory .sertor sertor-rag` (e.g.
  `uv run --directory .sertor sertor-rag search "<query>"`). It is the supported entry point — see
...
by hand**: run `uv run --directory .sertor sertor-rag search` or use the MCP search/navigation tools,
...
- `uv run --directory .sertor sertor-rag memory search "<query>"` — full-text search over archived
  sessions ("did we discuss X?").
- `uv run --directory .sertor sertor-rag memory list` / `… memory show <key>` — browse and read an
  archived session.
- `uv run --directory .sertor sertor-rag memory archive` — capture the current sessions (also runs
