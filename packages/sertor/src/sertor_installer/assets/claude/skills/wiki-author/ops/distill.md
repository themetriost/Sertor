...
- **from conversation**: a **targeted past session** retrieved from the episodic archive, condensed, then
  ...
  archive** (feature 036): `uv run --project .sertor sertor-rag memory list` (or `… memory search <query>`)
  to find the right `session_key`, then `uv run --project .sertor sertor-rag memory show <session_key>`
...
1. **Source material.** The work just completed (touched code + the `experiment` record just written), or — in
   ...
   from the archive via `uv run --project .sertor sertor-rag memory show <session_key>` (decisions, concepts, outcomes), then condensed.
