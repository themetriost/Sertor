...
Every Sertor-authored governance surface produced for `claude` MUST be produced for `copilot-cli`
too, with a valid container (possibly a different path), or be a declared gap. No silent omission
(FR-011). Plus: Claude and Copilot CLI governance can coexist on one repo (different containers).
The VS Code (`copilot`) target was removed (FEAT-012); parity is verified on the two remaining.
"""
...
def test_copilot_cli_covers_claude_surfaces(tmp_path: Path):
    """Copilot CLI logical surfaces ⊇ Claude logical surfaces (no silent omission, SC-002)."""
    claude = _logical_surfaces("claude", tmp_path / "a")
    copilot = _logical_surfaces("copilot-cli", tmp_path / "b")
    missing = claude - copilot
    assert not missing, f"Copilot CLI is missing Claude surfaces (undeclared gap): {missing}"


def test_claude_and_copilot_cli_coexist_on_one_repo(tmp_path: Path):
    ...
    rc_copilot = execute_governance_plan(
        build_governance_profile(tmp_path, assistant="copilot-cli"), runner=FakeSpecifyRunner()
    )
...
def test_no_silent_gap_report(tmp_path: Path):
    ...
    plan = build_governance_plan(build_governance_profile(tmp_path, assistant="copilot-cli"))
