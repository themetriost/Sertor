...
already broken, with nothing to change on your side.

> **Upgrade — bring the host fully to `0.4.2`, naming each capability:**
>
> ```
...
> ```
>
> **What actually changed here is the `rag` capability and the `.sertor/` runtime** — specifically the
> MCP server and its dependency bounds. The `wiki` capability and `sertor-flow` carry **no** changes;
> their commands are listed so every host ends on the same base. Name each capability explicitly: a
> bare `sertor upgrade` has been observed to refresh only one capability on a host that has several
> while still exiting green (open defect). `sertor` is not on your `PATH` — it is invoked through
> `uvx --from`, as above. Nothing you configured is touched.
>
> **On the fourth command, which deserves a word.** `uv sync --project .sertor --upgrade` re-resolves
> your runtime, and between 2026-07-28 and this release that re-resolution is what *broke* hosts: it
> replaced a safe SDK 1.x with the new major, which older Sertor versions could not import. From
> 0.4.2 the same command is safe — the server runs on both lines — and it is how an already-broken
> host gets back its tools.

### Fixed

- **The MCP server starts on both supported SDK lines — this is the release that heals an affected
  host.** The MCP SDK published **2.0.0 on 2026-07-28**, removing `mcp.server.fastmcp`, the module the
  server was built on. Any host that resolved dependencies after that date got a server that **dies at
  import**: not «some tools missing» but **no tools at all**, while `sertor-rag doctor` stayed green
  (its `mcp` check reads the registration in `.mcp.json`, not the startup). The server now obtains its
  machinery from either line, so a host whose lock already froze the new major is healed by updating
  Sertor — **no local patch, no lock surgery**.
- **A failure the system can describe now reaches your agent with its diagnosis.** On the new SDK line
  only an error declared *anticipated* carries its message to the model; anything else is rendered
  generically by design. Sertor's anticipated failures — missing index, unreachable embedding provider,
  missing optional extra, locked index, missing code-graph — are now declared as such, so an agent is
  told «graph not built: run index first» instead of «the tool failed». Conversely, the raw text of an
  unexpected internal exception no longer travels to the client on that line; it stays in the
  observability event and on stderr, where it belongs.
- **Dependency bound raised to `mcp>=1.2,<2.3`**, pinned to the minor that was actually measured. The
  upper bound by major stays: raising it is a decision, not a default.

### Changed

- **`docs/troubleshooting.md`** — the entry for «no MCP tools at all, but `doctor` is green» no longer
  explains how to live with the defect: it states that 0.4.2 fixes it, who was affected (including
  long-installed hosts, because our own upgrade procedure re-resolves), and why you should **not** pin
  the SDK yourself in `.sertor/pyproject.toml` — that file is regenerated by the installer.

### Verification — how this release was checked, and what was NOT checked

- **The server was exercised over the protocol**, by a real MCP client against the server running as a
  process: handshake, all ten tool names, payload shape, and the diagnostic content of an anticipated
  failure. This verification did not exist before; its absence had already let a wrong conclusion into
  our own backlog.
- **Both SDK lines run the MCP test suite in CI**, and the step asserts *which* line is active before
  running — a branch no execution walks through breaks in silence.
- **An affected host was measured healing on a throwaway machine**: the installation smoke plants the
  condition (the frozen major), **asserts the server cannot start**, upgrades, then asserts it can.
  Without that first assertion the outcome would pass for free.
- **The installation smokes now import the server.** Before this release none of them could see a host
  receiving zero tools, because `doctor` stays green and `search` goes through the CLI.
- **Not measured, and stated rather than presumed:** request cancellation, timeouts, very large
  payloads and concurrent calls on the new SDK line. Also still open: `doctor` reports the MCP
  registration, not the startup — so it remains capable of showing green on a server that cannot
  start. That is the check which should have caught this defect, and it is not fixed here.
