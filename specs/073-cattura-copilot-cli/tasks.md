...
→ dipende da: nessuno
- [x] Aggiungi il campo `copilot_session_dir: Path` con default `Path.home() / ".copilot" /
...
- [x] Aggiungi la lettura del nuovo campo in `load()` (o nella logica di lettura env esistente),
...
- [x] Posiziona il campo nella sezione `memory_*` di `Settings`, immediatamente dopo
      `claude_projects_dir`; nessun default hardcodato altrove (Principio VIII).
- [x] Verifica: `Settings()` costruito senza env → `copilot_session_dir == Path.home() /
...
→ dipende da: TASK-S01
- [x] Aggiungi test: `Settings()` senza env → `copilot_session_dir` è `~/.copilot/session-state`
      (verifica path atteso come `Path.home() / ".copilot" / "session-state"`).
- [x] Aggiungi test: con `SERTOR_MEMORY_COPILOT_SESSION_DIR=/tmp/fake` nel env →
      `copilot_session_dir == Path("/tmp/fake")`.
- [x] Verifica che il campo sia **distinto** e indipendente da `claude_projects_dir`
      (nessun aliasing, override di uno non influenza l'altro).
- [x] Test `not cloud`, offline (solo lettura env).
...
→ dipende da: TASK-S01
- [x] **Docstring di modulo**: dichiara la versione di Copilot CLI verificata (**1.0.63**,
...
- [x] **Costante di mapping** (host-specific vocabulary, confinata nel modulo):
      ```python
...
- [x] **Classe `CopilotCliCaptureAdapter`** — implementa la porta `TranscriptCaptureAdapter`
      ...
      ```
- [x] **`list_sessions()`** (FR-004/006/010/011/019, contratto §list_sessions):
...
- [x] **`read_session(ref)`** (FR-007/008/009, contratto §read_session):
...
- [x] **Helper puri** (confinati nel modulo, host-specific vocabulary — data-model §Helper):
...
- [x] **Crea `tests/unit/test_copilot_capture.py`** (NUOVO) con fixture di directory session-state:
...
- [x] Aggiorna `_VALID_MEMORY_ADAPTERS = ("claude-code", "copilot-cli")` (riga 29): aggiungi
      `"copilot-cli"` (data-model §Selettore, REQ-001/FR-001).
- [x] Nella funzione `build_capture_adapter`, dopo il ramo `claude-code` (invariato), aggiungi:
      ```python
...
- [x] Il ramo `claude-code` resta **invariato** nel comportamento (non-regressione FR-003/SC-010):
      encoding path + `claude_projects_dir` identici a prima; default `"claude-code"` preservato.
- [x] Il `ConfigError` per valore ignoto ora include **entrambi** i valori ammessi nel messaggio:
      `"(allowed: claude-code, copilot-cli)"` (REQ-002/FR-002).
- [x] Verifica: `build_capture_adapter` con `memory_adapter="copilot-cli"` → istanza
...
- [x] Aggiungi test: `SERTOR_MEMORY_ADAPTER=copilot-cli` + `SERTOR_MEMORY=true` →
      `build_capture_adapter(settings)` ritorna un'istanza `CopilotCliCaptureAdapter`
      (SC-001/US2-AC1).
- [x] Aggiungi test: `SERTOR_MEMORY_ADAPTER=claude-code` (default) → `ClaudeCodeCaptureAdapter`
      (non-regressione SC-010/FR-003/US2-AC3).
- [x] Aggiungi test: valore ignoto (es. `"gemini"`) → `ConfigError` con messaggio che contiene
      `"claude-code"` **e** `"copilot-cli"` (REQ-002/FR-002/US2-AC2).
- [x] Verifica che con `SERTOR_MEMORY=false` → `build_memory_archiver` ritorna `None` e il nuovo
      ramo `copilot-cli` **non** viene eseguito (additività, RNF-3/SC-011).
- [x] Tutti `not cloud`, Settings mock, nessun adapter reale che legga file.
...
**File**: `tests/unit/test_composition.py`
→ dipende da: TASK-US1-01
- [x] Test: `SERTOR_MEMORY_ADAPTER` non impostato (env assente) → `settings.memory_adapter ==
      "claude-code"` → `ClaudeCodeCaptureAdapter` selezionato (FR-003/SC-010, US2-AC3).
- [x] Test: `_VALID_MEMORY_ADAPTERS` contiene esattamente `("claude-code", "copilot-cli")` — nessun
      valore in più o in meno (contratto §Selettore).
- [x] Test: il comportamento di `build_memory_archiver` con `memory_enabled=True` e adapter
...
**File**: `tests/unit/test_settings.py`
→ dipende da: TASK-S01
- [x] Test: `Settings()` default → `memory_adapter == "claude-code"` (default invariato, FR-003).
- [x] Test: `SERTOR_MEMORY_ADAPTER=copilot-cli` → `settings.memory_adapter == "copilot-cli"`
      (FR-001/REQ-001).
- [x] Test: `SERTOR_MEMORY_ADAPTER=sconosciuto` → `settings.memory_adapter == "sconosciuto"` (la
...
→ dipende da: TASK-F01
- [x] Verifica che il test `test_read_session_discards_non_dialog_events` copra esplicitamente
...
- [x] Aggiungi se mancante: `test_read_session_transformed_content_ignored`:
...
**File**: `tests/unit/test_copilot_capture.py`
→ dipende da: TASK-F01
- [x] Aggiungi `test_paths_match_exact_cwd`:
      `_paths_match("/repo", "/repo", None)` → `True` (match esatto su cwd).
- [x] Aggiungi `test_paths_match_gitroot_contains_project`:
      `_paths_match("/repo", "/repo/sub", "/repo")` → `True` (gitRoot è antenato, DA-CM-4).
- [x] Aggiungi `test_paths_match_cwd_is_ancestor`:
...
- [x] Aggiungi `test_paths_match_both_none`:
      `_paths_match("/repo", None, None)` → `False` (DA-CM-2).
- [x] Aggiungi `test_list_sessions_multi_project_isolation`:
      ...
      (SC-009/US4-AC1).
- [x] Tutti test su path **sintetici** (stringhe, non path reali su disco) per garantire
...
- [x] In `test_copilot_capture.py`: `test_absent_source_dir_list_sessions_empty`:
...
- [x] In `test_copilot_capture.py`: `test_source_dir_empty_list_sessions_empty`:
...
- [x] In `test_composition.py`: `test_memory_off_copilot_adapter_selected_no_files_read`:
...
- [x] Documentale: verifica che `quickstart.md` contenga la nota cloud-sync (§6 Privacy &
...
**File**: `tests/unit/test_copilot_capture.py`
→ dipende da: TASK-F01
- [x] `test_list_sessions_idempotent`:
...
- [x] `test_read_session_unknown_event_fields_no_crash`:
...
- [x] `test_read_session_missing_data_field_no_crash`:
      Evento `user.message` senza campo `data` → saltato (nessun crash, RNF-1/FR-020/US6-AC2).
- [x] `test_read_session_content_not_string_no_crash`:
      Evento `assistant.message` con `data.content={"nested": "obj"}` → saltato (DA-CM-1
      fallback, RNF-1).
- [x] `test_read_session_entirely_unknown_format_returns_empty_turns`:
...
- [x] Verifica che `quickstart.md` §3 menzioni esplicitamente che l'hook `SessionEnd` già
...
- [x] In `test_composition.py`: aggiungi un test di integrazione sottile che simula il flusso
...
- [x] In `test_composition.py`: con `SERTOR_MEMORY=false` → `build_memory_archiver` ritorna
      `None` → hook diventa no-op (US7-AC2/SC-011/RNF-3).
- [x] Tutti `not cloud`, Settings mock, nessun Copilot reale.
...
- [x] Esegui `uv run pytest tests/unit/test_copilot_capture.py -v` → verde (tutti i nuovi test).
- [x] Esegui `uv run pytest tests/unit/test_settings.py -v` → verde (incluse le estensioni S01/S02).
- [x] Esegui `uv run pytest tests/unit/test_composition.py -v` → verde (incluse le estensioni
...
- [x] Esegui `uv run pytest -m "not cloud" tests/` → verde (suite completa non-cloud).
...
→ dipende da: tutti i task delle Fasi 0–8
- [x] Verifica che **nessuno** dei seguenti file sia stato modificato (RNF-5/SC-003):
...
- [x] Spot check comportamenti CLI invariati:
...
- [x] Verifica che con `SERTOR_MEMORY_ADAPTER=claude-code` (default): il nuovo modulo
...
- [x] Verifica nei test (o aggiungendo un test dedicato) che gli eventi emessi dall'adapter
...
- [x] Verifica che i 4 eventi abbiano il campo `adapter_kind="copilot-cli"` (distinzione dalla
      sorgente Claude nei log condivisi).
- [x] Verifica che un guasto nell'emissione dell'evento (es. `log_event` lancia eccezione) non
