...
- [ ] Aggiungi `SuiteNotFoundError(SertorError)`: campo `path: str`, messaggio azionabile
      «suite non trovata in `<path>` — crea la suite con `sertor-rag eval add-case`».
- [ ] Aggiungi `SuiteValidationError(SertorError)`: campo `case_index: int`, `detail: str`,
      messaggio che identifica il caso offendente per indice e contenuto parziale (REQ-004).
- [ ] Aggiungi `SuiteWriteError(SertorError)`: campo `path: str`, messaggio che segnala
      l'impossibilità di serializzare in modo sicuro (round-trip fallito, DA-a).
- [ ] Aggiungi `RegressionDetected(SertorError)`: campo `verdict: RegressionVerdict` (forward
...
- [ ] Verifica: ogni nuovo errore è sottoclasse di `SertorError`; nessun import esterno in
...
- [ ] Aggiungi campo `eval_dir: Path` con default `Path("eval")`, letto da `SERTOR_EVAL_DIR`
      (stesso pattern di `index_dir`; usa `Path(os.getenv(..., "eval"))`).
- [ ] Aggiungi campo `eval_tolerance: float` con default `0.0`, letto da `SERTOR_EVAL_TOLERANCE`
      (usa `float(os.getenv(..., "0.0"))`).
- [ ] Verifica: i default non sono hardcodati in nessun componente diverso da `Settings`
...
- [ ] Aggiungi dataclass `QueryOutcome` (frozen=True) con campi `query: str`,
...
- [ ] Estendi `EvalReport` con campo `per_query: tuple[QueryOutcome, ...] = ()` (default vuoto,
      retrocompatibile — RNF-2; i 2 test strict e `test_baseline_quality` continuano a passare
      senza modifiche).
- [ ] Aggiorna la funzione `evaluate` per popolare `per_query`: per ogni `(query, expected)` del
...
- [ ] Verifica che i test esistenti (`test_baseline_quality.py`, test strict in
...
→ dipende da: TASK-001
- [ ] Crea il package `src/sertor_core/services/eval/` con `__init__.py` (può riesportare le
      entità pubbliche; non obbligatorio per ora).
- [ ] Definisci `EvalCase` (frozen dataclass): `query: str`, `expected: tuple[str, ...]`,
...
- [ ] Definisci `EvalSuite` (frozen dataclass): `cases: tuple[EvalCase, ...]`. Metodi:
...
- [ ] Definisci `Baseline` (frozen dataclass): `hit_rate: dict[int, float]`, `mrr: float`,
      `queries: int`, `provider: str`, `recorded_at: str` (ISO-8601 UTC, informativo).
- [ ] Definisci `MetricDelta` (frozen dataclass): `name: str`, `current: float`,
      `baseline: float`, `delta: float`, `regressed: bool`.
- [ ] Definisci `RegressionVerdict` (frozen dataclass): `verdict: str` (`"pass"` |
...
- [ ] Definisci `PathValidation` (frozen dataclass): `checked: tuple[str, ...]`,
      `missing: tuple[str, ...]`, `index_available: bool`.
- [ ] Nessun import di SDK esterni; solo stdlib e `engines.evaluation` per `GroundTruth`.

### TASK-005 — Implementa `suite_io.py`: load/write/add/amend [P]
**File nuovo**: `src/sertor_core/services/eval/suite_io.py`
→ dipende da: TASK-004
- [ ] Implementa `load_suite(path: Path) -> EvalSuite` via `tomllib.load` (stdlib Python 3.11+):
...
- [ ] Implementa il **serializzatore TOML a mano** (research DA-a, Principio II/III — no `tomli-w`):
...
- [ ] Implementa `write_suite(path: Path, suite: EvalSuite) -> None` (non-distruttivo,
...
- [ ] Implementa `add_case(path: Path, case: EvalCase) -> None`: carica la suite esistente (o
      crea vuota se assente), aggiunge il caso se non già presente (dedup su `query`), chiama
      `write_suite`.
- [ ] Implementa `amend_case(path: Path, query: str, **kwargs) -> None`: carica, trova il caso per
      `query` (o `SuiteNotFoundError` se assente nella suite), aggiorna i campi specificati,
      chiama `write_suite`.
- [ ] Solo stdlib: `tomllib`, `pathlib`, `dataclasses`; nessun import da `composition.py`.

### TASK-006 — Implementa `baseline_io.py`: load/write baseline [P]
**File nuovo**: `src/sertor_core/services/eval/baseline_io.py`
→ dipende da: TASK-004
- [ ] Implementa `load_baseline(path: Path) -> Baseline | None`: file assente → `None` (gestito
...
- [ ] Implementa `write_baseline(path: Path, baseline: Baseline) -> None`: serializzatore TOML a
...
- [ ] `write_baseline` crea le cartelle intermedie (`path.parent.mkdir(parents=True, exist_ok=True)`).
...
→ dipende da: TASK-004
- [ ] Implementa `compare_to_baseline(report: EvalReport, baseline: Baseline | None,
...
- [ ] Verifica: la funzione è deterministica (stesso input → stesso output); nessun import I/O.
...
- [ ] Implementa `run_evaluation(engine: QueryableEngine, suite: EvalSuite,
...
- [ ] Implementa `validate_paths(paths: tuple[str, ...], indexed_paths: frozenset[str] | None)
...
- [ ] Solo import da `engines.evaluation` e `services.eval.*`; nessun import di composition o
...
- [ ] Aggiungi `build_eval_runner(settings: Settings)` che restituisce un callable (o un oggetto
...
- [ ] Aggiungi `build_indexed_docs(settings: Settings) -> frozenset[str] | None`: carica
...
- [ ] Verifica: i nuovi `build_*` sono cablati esclusivamente dalla composition (Principio I/VIII);
      nessun import di questi nel core o nei servizi.
- [ ] Aggiorna `__init__.py` del package `sertor_core` se ri-esporta le factory pubbliche.

### TASK-010 — Formatter output `eval` in `cli/output.py` [P]
**File**: `src/sertor_core/cli/output.py`
→ dipende da: TASK-003, TASK-004, TASK-007
- [ ] Aggiungi `format_eval_report(report: EvalReport, kinds: tuple[str | None, ...],
...
- [ ] Aggiungi `format_comparison(reports: tuple[tuple[str, EvalReport], ...],
      json_mode: bool) -> str`: tabella affiancata `metric / label1 / label2 / …` (pattern
      `--compare` in `contracts/cli-eval.md`). Funzione pura.
- [ ] Aggiungi `format_regression_report(verdict: RegressionVerdict, json_mode: bool) -> str`:
      compatta (`PASS`/`REGRESSED` + deltas); usabile standalone se serve. Funzione pura.
- [ ] Aggiungi `format_path_validation(pv: PathValidation, json_mode: bool) -> str`: lista
      `checked`/`missing`/`index_available`. Funzione pura.
- [ ] Verifica: nessuna delle funzioni ha side-effect; compatibilità informativa umano↔JSON (SC-002).

### TASK-011 — Sottocomando `eval` nella CLI (`cli/__main__.py`)
**File**: `src/sertor_core/cli/__main__.py`
→ dipende da: TASK-009, TASK-010
- [ ] Aggiungi sottocomando `eval` al parser argparse con sub-azioni annidate (stesso pattern del
...
- [ ] Implementa `_cmd_eval_run(args, settings)`: segui il flusso del contratto
...
- [ ] Implementa `_cmd_eval_add(args, settings)`: carica suite (o crea se assente), valida i path
...
- [ ] Implementa `_cmd_eval_validate(args, settings)`: chiama `build_indexed_docs` + `validate_paths`
      su i path forniti; restituisce `PathValidation` umano/JSON; exit 0 sempre (è verifica, non gate).
- [ ] Aggiorna il blocco `except SertorError` in `main()` per catturare anche `RegressionDetected`,
...
- [ ] Verifica: il CLI è thin (nessuna logica di retrieval; solo parsing → factory → format); exit
...
- [ ] `test_suite_io.py`: round-trip write→read identico al sorgente (REQ-011); query con `"`
...
- [ ] `test_baseline_io.py`: round-trip `write_baseline`→`load_baseline` identico; file assente →
      `None`; `recorded_at` ISO-8601 presente e non vuoto.
- [ ] `test_regression.py`: `compare_to_baseline(report, None, 0.0)` → `"no-baseline"`, exit 0;
...
- [ ] `test_eval_runner.py`: `run_evaluation` con engine mock (stile `tests/fixtures/mocks.py`)
...
- [ ] Tutti i test: `not cloud`, no rete, funzioni pure testabili senza I/O dove possibile.
...
- [ ] `test_output_eval.py`: funzioni pure — `format_eval_report` con `per_query` popolato produce
...
- [ ] `test_cli_eval.py`: stile `test_cli_search.py` / `test_cli_memory.py` — usa `subprocess` o
...
- [ ] Verifica gate exit-code (SC-004): il gate `RegressionDetected` produce exit 1 in modo
      riproducibile con suite e baseline fisse.

### TASK-014 — Migrazione fixture dogfood → `eval/suite.toml`
**File nuovo**: `eval/suite.toml`
→ dipende da: TASK-005
- [ ] Crea `eval/suite.toml` a root del repo trasformando `tests/fixtures/ground_truth.py`
...
- [ ] Commento in testa al file: `# Sertor eval suite (dogfood example). Versioned project data —
      no secrets. kind ∈ {"symbol","nl"} (optional). Paths are repo-root-relative POSIX.`
- [ ] Verifica round-trip: `tomllib.load(open("eval/suite.toml", "rb"))` non solleva; ogni `case`
      ha `query` e `expected` non vuoti (eseguibile come micro-script di verifica durante il task).
- [ ] Il fixture `tests/fixtures/ground_truth.py` resta **invariato** (i 2 test strict continuano
...
→ dipende da: TASK-002
- [ ] Aggiungi in entrambi i template (sezione commentata, stile delle manopole esistenti come
      `SERTOR_EMBED_CACHE`):
      ```
      ...
      ```
- [ ] Verifica: le righe sono commentate di default (feature non attiva di default — REQ-062 additività);
      nessun segreto nei template (RNF-6).
- [ ] Controlla che `test_packaging.py` (integration, se esegue il check di coerenza template) non
...
- [ ] Crea la skill `eval-suite-author` (directory `.claude/skills/eval-suite-author/`) seguendo
...
- [ ] Il corpo della skill deve esplicitare il confine D↔N (REQ-023/RNF-4): solo i casi
...
- [ ] La skill deve gestire il caso «corpus non indicizzato» (REQ-022): se `validate-path` o
...
- [ ] Corpo host-agnostico (Principio X): nessun riferimento a path Sertor-specifici; nessun
      nome-modello hardcodato (regola parità dual-target da FEAT-001 epica debito-tecnico).
- [ ] Verifica invariante (US2 AC4): il run deterministico (`eval run`) non dipende dalla skill;
...
- [ ] Individua il plan-builder `build_rag_plan` (o equivalente) nell'installer del pacchetto
      ...
      come `wiki-author`).
- [ ] Verifica che `sertor install rag` (dry-run) includa la skill nel piano generato; aggiorna
      `sertor_owned_paths` se la funzione esiste per garantire la copertura upgrade/uninstall.
- [ ] Verifica che il test di parità asset (se presente `test_assets_copilot_parity.py` o
...
- [ ] `test_cli_eval_compare.py`: `eval run --compare baseline,hybrid` con due engine mock →
      exit 0; output affiancato con entrambi i label; un evento `eval` per engine (verificabile
      con `caplog` o mock `log_event`).
- [ ] `test_skill_eval_author.py` (verifica statica/strutturale, non live LLM): il file
...
- [ ] Crea la skill `eval-feedback` (directory `.claude/skills/eval-feedback/`) con il pattern
...
- [ ] La skill deve garantire che **nessun giudizio venga inferito o persistito senza azione
      esplicita** (REQ-051): ogni modifica alla suite richiede conferma esplicita dell'utente.
- [ ] La skill gestisce il caso «query senza caso corrispondente nella suite» (REQ-052): offre di
      creare un nuovo caso con `eval add-case`.
- [ ] Corpo host-agnostico (Principio X); nessun nome-modello hardcodato.
- [ ] Verifica (US3 AC2): la skill non ha accesso a modalità «automatica»; ogni azione di
      scrittura passa dal CLI (vehicle).

### TASK-020 — Skill `eval-feedback` cablata nell'installer (debito completamento, US5/P3)
**File**: `packages/sertor/src/sertor_installer/` (installer rag plan builder)
→ dipende da: TASK-019
- [ ] Aggiungi `eval-feedback` al piano di `build_rag_plan` con lo stesso pattern di TASK-017.
- [ ] Aggiorna `sertor_owned_paths` se necessario.
- [ ] Verifica parità asset (nessun path/slash-command Sertor-specifico).

### TASK-021 — Test unitari: skill feedback e invarianti US3 [P]
**File nuovo**: `tests/unit/test_skill_eval_feedback.py`
→ dipende da: TASK-019
- [ ] Verifica statica: `eval-feedback/SKILL.md` esiste; contiene riferimento esplicito al vehicle
...
- [ ] Verifica che `amend_case` in `suite_io.py` sia richiamabile con i parametri che la skill
...
- [ ] Test `@integration` `not cloud` che usa Chroma locale (mock o istanza temporanea) e la suite
...
- [ ] Test `@integration` `not cloud` per `add-case` + `validate-path` con indice reale (o manifest
      temporaneo): verifica SC-003 (suite non vuota dopo aggiunta) e SC-010 (suite assente → exit 1).
- [ ] Tutti i test superano con `uv run pytest -m "not cloud" tests/integration/test_eval_gate.py`.
...
- [ ] Verifica **additività** (REQ-062/SC-009): con leve di default (`SERTOR_EVAL_DIR=eval`,
...
- [ ] Verifica che `EvalReport` senza il campo `per_query` (i test esistenti costruiscono
...
→ dipende da: TASK-009
- [ ] Verifica che `build_eval_runner` e `build_indexed_docs` siano esportati coerentemente con
      le altre factory pubbliche (pattern `build_facade`, `build_indexer`, ecc.).
- [ ] Aggiorna il docstring di `composition.py` per citare le nuove factory.
- [ ] Se `src/sertor_core/__init__.py` ri-esporta le factory, aggiungici `build_eval_runner` e
      `build_indexed_docs`; altrimenti lascia invariato (dipende dalla convenzione del package).
- [ ] Esegui `uv run pytest tests/unit/` per la suite completa: tutti i test passano (inclusi i
