...
- [ ] **T-000** Leggere i file hook statici oggi in formato Claude e annotare i difetti esatti
...
- [ ] **T-001** Leggere gli script hook e il seam esistente
...
- [ ] **T-002** Leggere i test Copilot esistenti e capire cosa verificano oggi
...
- [ ] **T-003** Eseguire la suite corrente (baseline verde) prima di modificare nulla
...
- [ ] **T-010** Aggiungere il campo `command_vehicle` (o modificare `_command_dir` / `_command_suffix`)
...
- [ ] **T-011** Test di non-regressione `AssistantProfile` — aggiornare/estendere i test esistenti
...
- [ ] **T-020** Correggere `render_prompt_file` per usare `agent:` invece di `mode:`
...
- [ ] **T-021** Correggere `render_custom_agent` per omettere `model:` sui target Copilot
...
- [ ] **T-022** [P] Test `render_prompt_file` — aggiornare i test esistenti e aggiungere asserzioni
...
- [ ] **T-023** [P] Test `render_custom_agent` — aggiornare i test esistenti e aggiungere asserzioni
  - File: `packages/sertor/tests/test_assets_copilot_guard.py` (e/o test di kit)
...
- [ ] **T-030** Generalizzare `_inner_commands` per riconoscere anche la forma piatta Copilot
...
- [ ] **T-031** Test retrocompatibilità merge — file esistente nel formato Claude annidato resta gestito
...
- [ ] **T-032** [P] Eseguire la suite completa del kit per confermare non-regressione
...
- [ ] **T-040** Aggiungere `HookEntrySpec` (dataclass frozen) nel kit
...
- [ ] **T-041** Implementare `render_copilot_hooks(events: list[HookEntrySpec]) -> dict`
...
- [ ] **T-042** Test unitari di `render_copilot_hooks` (offline, stdlib `json`)
...
- [ ] **T-050** Modificare `_build_copilot_wiki_plan` per usare `render_copilot_hooks` al posto
...
- [ ] **T-051** Modificare `install_rag.py` per usare `render_copilot_hooks` al posto dell'asset
...
- [ ] **T-052** Eliminare gli asset statici in formato Claude
...
- [ ] **T-053** Test di integrazione: plan wiki Copilot produce hook JSON nativo (non il vecchio formato)
...
- [ ] **T-054** [P] Test non-regressione target Claude (gate duro SC-010)
...
- [ ] **T-060** Aggiungere parametro `-Assistant` con default `claude` (non-regressione FR-040)
...
- [ ] **T-061** Implementare la resa output nativa per evento e assistente (no dual-field FR-011)
...
- [ ] **T-062** Aggiornare i comandi nel wiring Copilot per passare `-Assistant copilot`
...
- [ ] **T-070** Aggiungere `-Assistant` e garantire exit 0 sempre su Copilot
...
- [ ] **T-071** Aggiornare i comandi nel wiring Copilot (PreToolUse) per passare `-Assistant copilot`
  - File: `packages/sertor/src/sertor_installer/install_rag.py`
...
- [ ] **T-075** Test `wiki-pending-check.ps1` con `-Assistant copilot` per evento Stop
...
- [ ] **T-076** [P] Test `wiki-pending-check.ps1` con `-Assistant copilot` per evento SessionEnd
  - Assert: nessun output su stdout (o output non contiene payload consumato da Copilot) — O5
  - Assert: exit 0

- [ ] **T-077** [P] Test `wiki-pending-check.ps1` con `-Assistant claude` (default) — non-regressione
...
- [ ] **T-078** [P] Test `sertor-rag-usage-check.ps1` con stdin malformato su Copilot → exit 0 fail-open
...
- [ ] **T-080** Creare `wiki-session-start.ps1` (estratto dall'inline in `wiki.hooks.json`)
...
- [ ] **T-081** Aggiornare `install_wiki.py`: piano Claude usa lo script estratto invece dell'inline
  - File: `packages/sertor/src/sertor_installer/install_wiki.py`
...
- [ ] **T-082** Aggiornare `_build_copilot_wiki_plan` per SessionStart per-famiglia
  - File: `packages/sertor/src/sertor_installer/install_wiki.py`
...
- [ ] **T-083** Test SessionStart CLI: voce hook è `type:"prompt"`, nessuna stringa nuda — SC-003 / FR-006
...
- [ ] **T-084** [P] Test SessionStart VS Code: voce hook è `type:"command"`, output JSON parsabile
...
- [ ] **T-085** [P] Test non-regressione SessionStart Claude (gate duro)
...
- [ ] **T-090** Separare il piano COMMAND per `copilot` e `copilot-cli` nel wiki
  - File: `packages/sertor/src/sertor_installer/install_wiki.py`
...
- [ ] **T-091** Test piano wiki `copilot-cli`: nessun COMMAND solo-prompt-file — SC-004 / FR-013
  - File: `packages/sertor/tests/test_install_wiki_copilot.py` (aggiornare)
...
- [ ] **T-095** Separare il piano COMMAND per `copilot` e `copilot-cli` nella governance
...
- [ ] **T-096** Test piano governance `copilot-cli`: nessun COMMAND solo-prompt-file — FR-014 / SC-004
...
- [ ] **T-097** [P] Eseguire guard dipendenza `sertor-flow` dopo T-095
...
- [ ] **T-100** Verifica end-to-end: prompt-file installato su `copilot` ha frontmatter `agent:`
  - File: `packages/sertor/tests/test_install_wiki_copilot.py` (aggiornare)
...
- [ ] **T-101** [P] Verifica end-to-end: custom-agent installato su `copilot` / `copilot-cli` non ha `model:`
  - File: `packages/sertor/tests/test_install_wiki_copilot.py` (aggiornare)
...
- [ ] **T-102** [P] Verifica end-to-end governance: prompt-file e custom-agent con frontmatter corretto
  - File: `packages/sertor-flow/tests/test_install_governance_copilot.py` (aggiornare)
...
- [ ] **T-110** Creare o estendere la suite di validità-schema hook Copilot in `packages/sertor/tests/`
...
- [ ] **T-111** Test anti-pattern: reintrodurre ogni difetto → fallisce (SC-007 scenario 1)
...
- [ ] **T-115** Creare suite di validità-schema frontmatter Copilot in `packages/sertor/tests/`
...
- [ ] **T-116** [P] Test validità-schema custom-agent (FR-023 / SC-007 scenario 3)
...
- [ ] **T-120** [P] Test che gli script hook per `copilot` emettono output conforme per evento (SC-002)
...
- [ ] **T-125** Test che il piano `copilot-cli` non ha COMMAND solo-prompt-file (SC-004)
...
- [ ] **T-130** Documentare e applicare marker `@pytest.mark.no_network` (o simile) ai nuovi test
...
- [ ] **T-140** Aggiornare `install_wiki.py` per dichiarare `[ASSUNTO-VSC]` nell'`InstallReport`
  - File: `packages/sertor/src/sertor_installer/install_wiki.py`
...
- [ ] **T-141** [P] Aggiornare `install_rag.py` e `install_governance.py` per dichiarare gap analoghi
  - File: `packages/sertor/src/sertor_installer/install_rag.py`
...
- [ ] **T-145** Aggiornare `specs/049-compatibilita-copilot/contracts/surface-mapping-and-gaps.md`
...
- [ ] **T-146** [P] Aggiornare `wiki/tech/assistant-targeting.md` (o equivalente) con i gap dichiarati
...
- [ ] **T-150** Eseguire la suite completa di tutti e tre i pacchetti
...
- [ ] **T-151** [P] Verificare che non esistano import di `sertor-core` / `sertor` in `sertor-flow`
  - `uv run pytest packages/sertor-flow/tests/unit/test_no_core_dependency.py
    packages/sertor-flow/tests/integration/test_install_without_core.py -q`
  - GATE DURO: SC-011

- [ ] **T-152** [P] Ruff lint sui file modificati
  - `uv run ruff check packages/sertor-install-kit/src/ packages/sertor/src/ packages/sertor-flow/src/ --select E,F,I,UP,B`
  - 0 errori

- [ ] **T-153** [P] Verifica idempotenza: eseguire due volte il piano Copilot sullo stesso `tmp_path`
