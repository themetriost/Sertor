...
→ dipende da: TASK-F01
- [ ] Aggiungi `FusedResults` alle entità riesportate dal package pubblico `sertor_core`
...
- [ ] Verifica: `from sertor_core import FusedResults` funziona senza errori; nessun import
...
→ dipende da: nessuno
- [ ] Aggiungi la frozen dataclass `FusedResults` accanto a `RetrievalResult` (data-model §1):
      ```python
      ...
      class FusedResults:
          ...
          def flatten(self) -> list[RetrievalResult]:
              ...
      ```
- [ ] `docs` contiene solo risultati `doc_type=DOC`; `code` solo `doc_type=CODE` (garantito
      dalla facade, non da un guard sull'entità — l'entità è puro dato).
- [ ] `flatten()` implementa l'interleave deterministico per rank (DA-c, research §DA-c):
...
- [ ] Garanzie `flatten()` da testare (library-contract.md):
...
- [ ] Aggiungi test in un file dedicato `tests/unit/test_fused_results.py` (o inline in
...
- [ ] **Nessun import di SDK esterni** in `domain/entities.py` (Principio I); la frozen
      dataclass è pure data, zero dipendenze dal domain verso l'esterno.
- [ ] Verifica: `domain/entities.py` importabile senza dipendenze esterne; i test esistenti di
...
- [ ] Cambia la firma di `search_combined`:
      ```python
      # PRIMA
      ...
      ```
- [ ] Sostituisci il corpo da `self._search(query, k, "both")` (budget condiviso) a due chiamate
...
- [ ] Conserva la gestione del fan-out multi-corpus (feature 010, data-model §2): quando ci sono
...
- [ ] `search_code` e `search_docs` **non vanno toccate** (FR-003/SC-002): solo `search_combined`
      cambia.
- [ ] Logging invariato per costruzione: i due `_search` per tipo emettono i propri eventi
...
- [ ] Aggiorna docstring di `search_combined` per rispecchiare il nuovo contratto (budget separato,
      ritorno `FusedResults`).
- [ ] Verifica: la firma aderisce al library-contract.md; nessun consumatore è ancora
...
- [ ] Aggiorna la firma di `fusion_coverage`:
      ```python
      # PRIMA
      ...
      ```
- [ ] Aggiorna il corpo per leggere `has_doc`/`has_code` dalle **due liste separate** (data-model §3):
      ```python
...
- [ ] `FusionCaseResult`/`FusionReport` (`models.py`) **non cambiano forma**: cambia solo la
      sorgente di `has_doc`/`has_code`. Se `models.py` è in un file separato, verificare che
      le entità siano importate, non ridefinite.
- [ ] Verifica: funzione pura deterministica (stesso input → stesso output); mockabile con
...
- [ ] Rimuovi `"search_combined"` da `_SURFACES` (data-model §4, research §superficie):
      ```python
      # PRIMA
...
- [ ] `_SurfaceEngine` (o la logica equivalente) non deve più wrappare `search_combined` come
...
- [ ] La chiamata a `fusion_coverage` già usa `facade.search_combined` come `search_fn`: ora che
...
- [ ] Aggiorna `emit_fused_eval_event` (o `emit_fused_eval_event`): i campi `surface_mrr` e
...
- [ ] Verifica: l'evento `fused_eval` ha esattamente le 2 chiavi in `surface_mrr`/`surface_hit3`;
...
→ dipende da: TASK-R01, TASK-F01
- [ ] Nella funzione `_cmd_search` (gestione `--type both`): sostituisci il consumo di
...
- [ ] `--type code` e `--type doc` restano invariati (SC-002): non toccarli.
- [ ] Nella funzione `_fused_baseline_from` (righe 687–704): aggiorna per iterare su
...
- [ ] Verifica: il CLI thin (nessuna logica di metrica nel CLI, Principio I); exit code coerenti;
...
→ dipende da: TASK-F01
- [ ] Aggiorna `format_search_results` (o la funzione che rende il combined) per accettare
...
- [ ] In modalità `--json` (`--type both`):
      ```json
      { "docs": [ {…} ], "code": [ {…} ] }
      ```
      Gemello del MCP (cli-search-combined.md). `--type code`/`--type doc` restano lista JSON
      come oggi (invariati).
- [ ] `search_code`/`search_docs` (`--type code`/`--type doc`) producono ancora una sezione unica
      (invariati — SC-002): non toccarli.
- [ ] Formato citabile `path#chunk` preservato (cli-search-combined.md §Invarianti).
- [ ] Funzione pura (zero I/O, zero side-effect): testabile senza adapter.
...
→ dipende da: TASK-R01, TASK-F01
- [ ] Il tool MCP `search_combined` ora ritorna un `dict` invece di `list[dict]`
      ...
      ```
- [ ] `_fmt(r)` è invariato (path/source/chunk/score/preview, formato citabile `path#chunk`).
- [ ] Una lista vuota → chiave esiste comunque con `[]` (forma sempre strutturata —
      mcp-search-combined.md §Una lista vuota).
- [ ] `_guard` invariato: persiste `mcp.search_combined.error` e ri-solleva (visibilità,
      Principio XII). Il log di superficie `mcp.search_combined` può riportare `docs`/`code` counts.
- [ ] `search_code`/`search_docs` e gli altri 4 tool invariati (solo `search_combined` cambia).
- [ ] Aggiorna docstring del tool `search_combined` e le `instructions` del server (righe 33–37
...
- [ ] **`test_retrieval_facade.py`** — aggiorna/aggiungi casi per `search_combined`:
...
- [ ] **`test_fusion.py`** — aggiorna per `search_fn: Callable[[str,int], FusedResults]`:
...
- [ ] **`test_fused_runner.py`** — aggiorna per le 2 superfici:
...
- [ ] **`test_regression_fused.py`** — verifica coerente con 2 superfici (non 3):
      - Il verdetto di regressione copre le 2 superfici + `fusion_coverage`.
      - Nessun delta per `search_combined` (rimosso).
- [ ] **`test_baseline_io_fused.py`** — round-trip con `[fused_baseline]` a 2 `[[fused_baseline.surface]]`
      (non 3):
...
- [ ] Tutti i test: `@pytest.mark.not_cloud` o nessun marker; zero rete.
...
- [ ] **`test_output_fused_eval.py`** — aggiorna il formatter per la resa a due sezioni:
...
- [ ] **`test_cli_fused_eval.py`** — aggiorna i test del percorso `--type both`:
...
- [ ] **`test_mcp_server.py`** — aggiorna i test del tool `search_combined`:
...
- [ ] **`tests/integration/test_end_to_end.py`** — aggiorna i casi che esercitano `search_combined`:
...
- [ ] Tutti i test: `not cloud`, deterministici; i test `@integration` usano corpus minimale locale.
...
- [ ] Esegui `uv run pytest -m "not cloud" tests/unit/` → verde (tutti i test unit, inclusi
...
- [ ] Verifica che i test esistenti IR (`eval run`, senza `--fused`) e graph-eval siano
...
→ dipende da: tutti i task della Fase 2
- [ ] Verifica che **nessun** file tra i seguenti sia stato modificato (RNF-1/SC-010):
...
- [ ] Verifica comportamenti CLI invariati (spot check manuale o via test):
...
- [ ] Documenta l'additività verificata come commento nel tasks.md o nel commit brief.

### TASK-P03 — Smoke test MCP con nuovo contratto (post-merge, pre-re-baseline)
→ dipende da: TASK-P01, TASK-P02
- [ ] Dopo il merge su master (o su branch pronto), riavvia il server MCP `sertor-rag`
      (serve codice nuovo, non solo indice — quickstart §4).
- [ ] Esercita il percorso del filtro metadata (regola standing, CLAUDE.md):
      ...
      - `find_symbol` su simbolo a posizione nota → riga coerente col file reale (freschezza
        code-graph).
- [ ] Un tool in errore → segnala esplicitamente (regola *errori-MCP = finding, mai rumore*);
