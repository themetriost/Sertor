...
→ dipende da: nessuno
- [x] Aggiungi il campo `memory_semantic_enabled: bool = False` letto da
...
- [x] Aggiungi il campo `memory_semantic_limit: int = 20` letto da `SERTOR_MEMORY_SEMANTIC_LIMIT`
      (default `20`, gemello di `episodic_limit`), tetto risultati della ricerca (REQ-011/FR-014).
- [x] Aggiorna il metodo `load()` (o la logica di lettura env) per includere i due nuovi campi,
      seguendo il pattern degli altri campi `SERTOR_MEMORY_*` già presenti.
- [x] Posiziona i due campi nella sezione `memory_*` di `Settings`, ordinati dopo i campi
      `memory_` esistenti; nessun default hardcodato altrove (Principio VIII).
- [x] Verifica: `Settings()` costruito senza env → `memory_semantic_enabled=False`,
...
→ dipende da: nessuno
- [x] Definisci `SemanticMemoryUnavailableError(SertorError)` come errore di dominio azionabile
...
- [x] **Nessun fallback silenzioso**: questo errore è sollevato dal consumer CLI quando
      `--semantic` è richiesto ma la factory ritorna `None` o l'indice è assente (data-model §Errore).
- [x] Il core (`MemorySemanticIndex`) resta non-fatale (degradazione via `hits=()` + warning):
      l'errore azionabile è sollevato **solo nel CLI** (confine D↔N — REQ-015 vs REQ-021).
- [x] Coerente con `SessionNotFoundError`/`InvalidTimeWindowError` già presenti nello stesso file.
- [x] Verifica: importabile da `sertor_core` come le altre eccezioni; nessun import circolare.
...
→ dipende da: TASK-S01
- [x] Definisci le quattro entità frozen dataclass di dominio (data-model §Entità, nessun SDK):
...
- [x] Implementa `MemorySemanticIndex` (componente concreto, **NO nuova porta** — Principio III):
      ```
      ...
      ```
- [x] **`search`** (REQ-009/010/011/012/FR-012..015, contratto §search):
...
- [x] **`index_session`** (REQ-006/030/031, contratto §index_session):
...
- [x] **`index_all`** (REQ-007/030, contratto §index_all):
...
- [x] **Collezione isolata** (REQ-017/FR-020, data-model §Namespacing):
...
- [x] **Privacy on-machine** (REQ-018/019, FR-021/022, RNF-1):
...
- [x] **Host-agnostico** (REQ-024/025/FR-027/028): nessun branch sull'assistente; opera su
      `memory.sqlite` indipendentemente dalla provenienza.
- [x] **Nessun import di SDK esterni**: dipende solo dalle porte `EmbeddingProvider`/`VectorStore`
      + entità/errori esistenti (Principio I); le scelte concrete sono in `composition.py`.
- [x] Aggiungi test unitari in `tests/unit/test_memory_semantic.py` (NUOVO, offline con mock):
...
- [x] Aggiungi la factory `build_memory_semantic_index(settings: Settings) -> MemorySemanticIndex | None`:
...
- [x] Aggiorna la firma di `build_memory_archiver`:
      ```python
...
- [x] L'iniezione avviene **solo in composition**: composition è l'unico luogo che conosce gli
      adapter concreti (Principio I). `MemoryArchiveService` non importa `build_*`.
- [x] Verifica: con `SERTOR_MEMORY=false` → `build_memory_semantic_index` ritorna `None` (gate);
...
- [x] Aggiungi il flag booleano `--semantic` al subparser `memory search` (DA-SS-3):
      ```
...
- [x] Routing nell'handler `_cmd_memory_search`:
...
- [x] Consuma `SemanticMemoryResults` (da TASK-F01): nessuna logica di mapping nel CLI thin
      (Principio I/VII); tutta la logica di formattazione in `cli/output.py` (TASK-US1-03).
- [x] Exit code: 0 = ricerca completata (anche con `hits=()`); 1 = leva spenta/indice assente
...
→ dipende da: TASK-F01
- [x] Aggiungi la funzione pura `format_semantic_results(results: SemanticMemoryResults,
      json: bool = False) -> str`:
...
- [x] Aggiungi la funzione pura `format_semantic_index_report(report: SemanticIndexReport,
      json: bool = False) -> str`:
...
- [x] Gemelle delle funzioni `format_archive_report`/`format_memory_results` già presenti
...
- [x] `memory search "query"` (senza `--semantic`) → percorso full-text invariato; `SemanticMemoryIndex`
      **non** costruito/chiamato (SC-004/REQ-013, US4-AC1).
- [x] `memory search "query" --semantic` con leva accesa e index mock → exit 0, output umano
      contiene snippet + score; `--json` → JSON con chiave `hits` (o simile) e 6 campi per hit
      (REQ-010, US1-AC2/AC3).
- [x] `memory search "query" --semantic` con `SERTOR_MEMORY_SEMANTIC=false` → exit 1,
      messaggio contiene `SERTOR_MEMORY_SEMANTIC` (REQ-015, US4-AC3/SC-005).
- [x] `memory search "query" --semantic` con `SERTOR_MEMORY=false` → exit 1, messaggio contiene
      `SERTOR_MEMORY` (REQ-002/FR-002, US2-AC2).
- [x] `memory search "query" --semantic --since T1 --until T2` → filtro temporale passato
      correttamente alla query (REQ-012, US1-AC1).
- [x] `memory search "query" --semantic` con `hits=()` → exit 0, output `(nessun risultato)` onesto
      (REQ-021, contratto §search edge case).
- [x] `format_semantic_results` con `hits` mock → output umano e JSON corretti (unit puro).
- [x] `format_semantic_index_report` → output umano e JSON corretti (unit puro).
- [x] Tutti `not cloud`, mock core (RNF-5).
...
→ dipende da: TASK-US1-01
- [x] `SERTOR_MEMORY=false`, `SERTOR_MEMORY_SEMANTIC=true` → `build_memory_semantic_index` ritorna
      `None`; nessun embedder né store costruiti; nessun embedding eseguito (REQ-001/002, US2-AC1/2).
- [x] `SERTOR_MEMORY=true`, `SERTOR_MEMORY_SEMANTIC=false` → `build_memory_semantic_index` ritorna
      `None`; `build_memory_archiver` riceve `semantic_index=None` → comportamento FEAT-001
      identico (REQ-005/FR-005/RNF-005, US2-AC1).
- [x] `SERTOR_MEMORY=true`, `SERTOR_MEMORY_SEMANTIC=true` → `build_memory_semantic_index`
      ritorna un'istanza `MemorySemanticIndex` (gate acceso, US2-AC3).
- [x] Con la semantica off: dopo aver simulato l'archiviazione di sessioni, nessun file/indice
...
- [x] Tutti `not cloud`, Settings mock, nessun adapter reale.
...
→ dipende da: TASK-S01
- [x] `Settings()` default → `memory_semantic_enabled=False` (default off, REQ-003/FR-003).
- [x] `Settings()` con `SERTOR_MEMORY=true` e senza `SERTOR_MEMORY_SEMANTIC` →
      `memory_semantic_enabled=False` (accendere la cattura non accende embedding, US2-AC3/SC-002).
- [x] `SERTOR_MEMORY_SEMANTIC_LIMIT` non impostato → `memory_semantic_limit=20` (default finito
      documentato, REQ-011).
- [x] I due campi sono **distinti** in `Settings`: verifica che non ci sia aliasing o derivazione
...
- [x] Aggiorna `MemoryArchiveService.__init__` per ricevere un parametro opzionale:
      ```python
...
- [x] Aggiorna `archive_all()` per chiamare `index_session` su ogni sessione **appena archiviata**,
...
- [x] Solo le sessioni **appena archiviate** (non già presenti, per idempotenza) scatenano
      `index_session`: coerente con `archive_all` che usa `INSERT OR IGNORE`.
- [x] Verifica: `memory archive` con semantica opt-in → sessione archiviata **e** indicizzata
...
- [x] Archivio una sessione con `semantic_index` mock iniettato → `index_session` chiamato una
      volta (auto-index, REQ-004, US3-AC1).
- [x] Senza `semantic_index` (None iniettato) → `archive_all` si comporta come FEAT-001: nessuna
      chiamata all'embedder (REQ-005, US2-AC1/RNF-005).
- [x] Embedding fallisce a fine archiviazione → la sessione è nel grezzo, warning loggato, run
      non abortito, `archive_all` ritorna normalmente (REQ-008, US6-AC4).
- [x] `index_session` chiamata due volte sulla stessa sessione (via `index_all` dopo auto-index) →
      seconda chiamata: `embedded=0`, `skipped=N`, zero chiamate embedder (incrementalità tra
      run, REQ-030/031, US3-AC2/AC4/SC-006).
- [x] Tutti `not cloud`, mock embedder/store/archivio, nessun file su disco (RNF-5).
...
- [x] `memory search "query"` (senza `--semantic`) con semantica opt-in (entrambe le leve accese) →
...
- [x] `memory search "query"` (senza `--semantic`) con semantica **off** → stessa cosa: full-text,
      nessun percorso semantico (SC-004).
- [x] Abilitare `SERTOR_MEMORY_SEMANTIC=true` non modifica il comportamento di `memory search`
...
- [x] `memory search "query" --semantic` con `SERTOR_MEMORY_SEMANTIC=false` → exit 1, messaggio
...
- [x] `memory search "query" --semantic` con indice semantico assente (leva accesa ma `index_all`
...
- [x] Il messaggio di errore **non** contiene risultati full-text mascherati da risultati semantici
...
→ dipende da: TASK-F01, TASK-S01
- [x] Con embedder mock (locale, nessuna rete) + store mock: `index_session` e `search` completano
...
- [x] Il provider locale (`glove`/`hash`) è il default da `SERTOR_EMBED_PROVIDER` (REQ-018/FR-021):
...
- [x] Con provider cloud configurato (mock che simula Azure embedder): il percorso funziona;
...
- [x] Tutti `not cloud`, nessun traffico reale; il test di isolamento rete è sulla coerenza del
...
- [x] Aggiungi il subcommand `memory index-semantic [--json]` (contratto §Vehicle CLI/backfill):
      ```
      ...
      ```
- [x] Handler `_cmd_memory_index_semantic`:
...
- [x] Idempotente per costruzione: una seconda esecuzione senza nuove sessioni → `embedded=0`,
      `skipped=N` (incrementalità, REQ-007/030/FR-009, US6-AC5/SC-006).
- [x] Exit 0 se l'operazione si completa (anche parzialmente con `errors>0`); exit 1 solo se
...
**File**: `tests/unit/test_memory_semantic.py` (estensione di TASK-F01)
→ dipende da: TASK-F01
- [x] Indice semantico assente alla query (store mock con collezione vuota) → `hits=()` + warning
      (REQ-021/FR-024, US6-AC1). Nessuna eccezione propagata al chiamante.
- [x] Provider giù a query-time (embedder mock che solleva eccezione) → stato vuoto/errore
      azionabile; il chiamante non va in crash (REQ-022/FR-025, US6-AC2).
- [x] Unità con embedding invalido nello store (payload mancante/malformato) → quella unità
      saltata con warning; le unità valide servite (REQ-023/FR-026, US6-AC3).
- [x] Tutti `not cloud`, nessuna rete.
...
- [x] `memory index-semantic` con semantica opt-in e archivio mock → `index_all` chiamato; output
      contiene `embedded`, `skipped`, `errors`; exit 0 (US6-AC5/REQ-007).
- [x] `memory index-semantic --json` → JSON con i tre campi (contratto §backfill).
- [x] `memory index-semantic` con leva spenta → exit 1, messaggio nomina
      `SERTOR_MEMORY_SEMANTIC=true` (contratto §backfill gate).
- [x] `memory index-semantic` senza nuove sessioni → `embedded=0`, exit 0 (idempotenza, SC-006).
- [x] Tutti `not cloud`, mock archivio e index.
...
- [x] Esegui `uv run pytest -m "not cloud" tests/unit/` → verde (inclusi i test pre-esistenti
...
- [x] Verifica che i test esistenti FEAT-001 (`test_memory_archive.py`), FEAT-002
...
→ dipende da: tutti i task delle Fasi 0–7
- [x] Verifica che **nessuno** dei seguenti file sia stato modificato (RNF-005/SC-011):
...
- [x] Verifica comportamenti CLI invariati (spot check):
...
- [x] Verifica che con `SERTOR_MEMORY_SEMANTIC=false` (default): nessun import del percorso
...
- [x] Esegui (o verifica nei test) che `memory_semantic_index` emesso da `index_all`:
...
- [x] Verifica che `memory_semantic_search` emesso da `search`:
...
- [x] Verifica che un guasto nell'emissione dell'evento non abortisca il risultato (REQ-028/FR-031):
...
- [x] Verifica coerenza con l'archivio (REQ-029/FR-032): cancellare/ricostruire l'indice non
