...
- **[[spec-010-query-congiunta-e-upsert-index]]** — Spec FEAT-010 (2026-06-10): pezzi D residui del wiki (query congiunta multi-collezione + `upsert-index` CLI), SpecKit specify+clarify completati. 4 decisioni dell'utente: fail-fast su embeddings eterogenei, configuration-driven multi-collezione, sommario multilinea → errore, fan-out solo per `search_combined`. Aggiunta regola contesto-nelle-domande in CLAUDE.md.
