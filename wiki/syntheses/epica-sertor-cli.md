...
## Domande Aperte (da Chiarire)

| DA | Domanda | Impatto |
|----| --------|---------|
| DA-1 | Nome pacchetto/comando CLI (es. `sertor`?) | Branding, comunicazione |
| DA-2 | Confine install/config/run — MVP installa ma configura/esegue dopo (FEAT-005/006 = Should)? | Scope MVP, sequenza feature |
| DA-3 | Governance (FEAT-007) come Should, non nel MVP? | Scope MVP, timeline |
| DA-4 | Distribuzione interna prima di PyPI (indice privato, git+url, wheel)? | CS-1, canale deploy |
| DA-5 | Vector DB realmente opzionale o condizionatamente obbligatorio per certi motori? | Design config, constraint motori |
| DA-6 | Set provider LLM del primo taglio (OpenAI, Anthropic, Azure Foundry, …)? | FEAT-005 dettaglio, supporto |
