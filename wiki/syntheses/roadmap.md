...
| A-07 | EVO | P1 | `search_docs` MRR 0.55 (leva missione) | `retrieval-qualita` E5-FEAT-003 | 🔄 **misura-prima fatta (2026-07-07).** Baseline live: `search_code` MRR 0.74 (robusto), `search_docs` MRR 0.55 (debole). **Gate rosso = regressione VERA** (stesso provider): corpus +59% (813→1292 doc) ha sepolto doc attesi. **Causa isolata: duplicazione di contenuto** — il bundle installer `assets/**` (byte-copie dei blocchi `CLAUDE.md`) rankano #1-2 e seppelliscono le `wiki/concepts/*.md` canoniche (step-ritual→rank 4, constitution→rank 6). `.claude` è già escluso, `assets` **no**. Calibrata 1 miss suite-narrowness (case fusion-coverage, `research.md` = risposta legittima → union 0.667→0.833). **Leva tracciata (2 piste):** (1) *quick win dogfood* — escludere `assets` da `SERTOR_EXCLUDE_PATTERNS`; (2) *fix prodotto* — dedup near-duplicate nel retrieval (ogni host). *(dettaglio: `wiki/log/2026-07-07.md`)* |
