...
<!-- EXEC:START -->
## ⚡ Executive summary (stato al 2026-07-26)

### 🔄 In progress
**🔴 PROSSIMO PASSO — E10-FEAT-045: il gate `wiki-guard` può essere INSODDISFACIBILE.** `scan` ancora al **mtime** del log; `gh pr merge` + `git pull` riscrivono i file mergiati, che diventano più recenti del giornale → allo `Stop` il gate blocca, e per soddisfarlo bisogna scrivere ancora nel wiki, il che riproduce la condizione al merge dopo. **Una sessione non può chiudersi sul proprio ultimo merge**; l'unica uscita è l'albero sporco. Sette occorrenze in un giorno sul nodo *Acta*. **Il nostro `pending=0` è una corsa vinta, non correttezza.** Rimedio nella forma del Principio XIV: **diff git dove l'ospite è un repo** (derivato dal fatto, già usato da `ritual-check`), **mtime dichiarato come proxy** dove non lo è (`scan` è host-agnostico *by design*). **Bivio aperto:** accettare anche un **giudizio registrato** come chiusura. *(E10-FEAT-048 «nominare i file» NON copre questo caso.)*
