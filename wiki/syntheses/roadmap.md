...
### 🔄 In progress
**🔨 IN LAVORAZIONE — E10-FEAT-045: il gate `wiki-guard` può essere INSODDISFACIBILE.** *Dove:* branch `123-feat-045-ancora-derivata-scan`, spec scritta (`specs/123-feat-045-ancora-derivata-scan/spec.md`, checklist 16/16). *Prossimo passo concreto:* chiudere il `clarify` sull'unica assunzione aperta (**A-2**), poi `plan`.
...
- **Assunzione aperta A-2** (decisione utente, in `clarify`): una voce di giornale **scritta ma non committata** vale come registrazione — necessario, perché allo `Stop` niente è ancora committato — ma **con quale scadenza?** Senza vincolo, un giornale lasciato non committato spegne il gate per giorni, cioè rende **legittima** la via d'uscita dell'albero sporco invece di toglierla.
