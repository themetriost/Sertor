...
| **Igiene della radice ospite (installer)** — meno file a root | L'utente (2026-06-12, su Kaelen) non gradisce `wiki.config.toml` e `.mcp.json` nella radice del progetto ospite | DUE fix lato installer: **(1)** `wiki.config.toml` → dentro `wiki/` (cartella autocontenuta; invocazioni `sertor-wiki-tools` con `--config wiki/wiki.config.toml --root .` in skill/hook/agente; retro-compat). **(2)** `.mcp.json`: VERIFICATO su doc Claude Code che il project-scope DEVE stare in root (non spostabile in `.claude/`, niente `mcpServers` in settings.json); via d'uscita = flag `--mcp-scope local` che registra il server in `~/.claude.json` via `claude mcp add` (zero file nel repo, ma config per-utente non versionata). Inevitabili a root: `.claude/`, `CLAUDE.md`, `wiki/`. | 👍 **da decomporre** (utente, 2026-06-12) |
