...
| A-09 | FIX | P1 | Hook POSIX story (promuovere E2-FEAT-010 da Could) | `sertor-cli` E2-FEAT-010 | 🔄 **quasi done su branch `095-portable-hooks-09`** — 8 hook riscritti in **Python portabile** (`uv run --no-project python`, zero dip `pwsh`); parità coi `.ps1` provata (gate) + smoke CI matrice ubuntu+windows; `.ps1` **ritirati** (single-impl DA-1); `upgrade` migra host legacy (file+wiring); **E10-FEAT-018 superata** (`host_env.py`/nota pwsh rimossi — fix, non mitigazione); doc utente aggiornata. `sertor-core` invariato. **Resta:** merge + migrazione live dogfood via `sertor upgrade` |
