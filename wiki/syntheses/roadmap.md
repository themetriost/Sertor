...
| **Installer: collocazione + modello mono/enterprise** (2 feature ORTOGONALI, separate su richiesta utente) | Radice ospite ordinata (asse **DOVE**) e modello di ownership team-vs-per-utente (asse **CHI**), tenuti distinti perché indipendenti | Decomposte in requirements (2026-06-12): **`igiene-radice-host`** (collocazione: `wiki.config.toml`→`wiki/`, `.sertor/` unica sede del runtime, meccanismo `--mcp-scope project\|local`, residenti inevitabili a root documentati) e **`installer-multiutente`** (ownership: **modalità mono-utente default / enterprise-team** che fissa i default — gitignore di `.sertor/`, scope MCP, onboarding del 2° dev; store condiviso MongoDB/PGVector documentato). DA-a..d risolte; resta DA-e (forma della modalità) per clarify. Vincolo verificato: `.mcp.json` project-scope DEVE stare in root (doc Claude Code). | 🔄 **requirements pronti** → `/speckit-specify` (utente, 2026-06-12) |
