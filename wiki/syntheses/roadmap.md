...
- **🎯 Il salto lungo regge: `v0.3.0 → master`, QUATTRO combinazioni** (ubuntu+windows × claude+copilot-cli) → **8 esiti su 8, tutte verdi**, zero `n/a` verificati leggendo i log. È la **prima risposta parziale a E15-FEAT-014** («quanto indietro regge l'aggiornamento?»): il limite noto passa da «una combinazione, una release indietro» a **«quattro combinazioni, quattro release indietro»**. Non la chiude — quella vuole la matrice da *ogni* versione — ma la domanda non è più senza risposta.
- **Il confine dichiarato, che vale più del numero:** l'host di prova è quello che il nostro installer produce; la configurazione in cui il pin ruppe da *Acta* — runtime fissato a un **tag immutabile** — resta quella che il nostro nodo **non ha** (terzo limite di [[dogfood-fidelity]]).
- **Rilascio annunciato e risposte pubblicate** in bacheca (canali *Releases* e *Feedback Sertor*, depositate). Le tre segnalazioni del 29/07 di *Acta* hanno finalmente risposta, con il **residuo nominato**: E10-FEAT-060 · E10-FEAT-063 · E10-FEAT-047 · E2-FEAT-023 restano aperti.
...
- **⚠️ Debito collaterale:** `testpaths = ["tests"]` → il comando che il `CLAUDE.md` dichiara *«gate vincolante pre-merge»* **non raccoglie** i ~600 test dei `packages/`. Copre 1385 test su **~2517**; la CI li esegue in 7 invocazioni separate.
- **È la messa in opera di due pagine già distillate e mai diventate meccanismo:** [[esito-sull-host-vs-forma-dell-asset]] (17/07) e il terzo limite di [[dogfood-fidelity]] (26/07).

**✅ E10-FEAT-065 — il `lint` non dichiara più rotto ciò che esiste** (segnalato da *Acta*, **consegnato su `master`**, merge `a58a6bf`/PR #254; fix provato dal vivo dal runtime installato). `iter_pages` esclude **giustamente** indice e partizioni di giornale (non sono contenuto: niente frontmatter, niente orfani), ma il `lint` costruiva **da lì** anche l'indice dei bersagli linkabili — *«è una pagina?»* usata per rispondere a *«esiste?»*. Riprodotto con la classe **più larga** di quella segnalata: rotti anche `[[log/index]]` e **`[[index]]`**, l'indice del wiki stesso. Rimedio: nuova `iter_linkable_files`, separata; le pagine si registrano per prime, così su collisione d'alias **vince la pagina**. **La sonda che riproduceva il difetto passa su tutte e cinque le forme** — stessa sonda, esito opposto; +8 test, incluso l'anti-vacuità (un link davvero penzolante continua a rompersi).

**✅ E10-FEAT-064 — `wiki-curator` non si ferma più su un asset che c'è** (segnalato da *Acta*, **consegnato su `master`**, merge `3d2d6d1`/PR #252). La guardia fail-loud era corretta ma scattava su un file **presente**: il playbook era riferito **per nome**, non per coordinata, mentre la riga dopo dichiara *«You do not have the skill's context»*. Rimedio per **suffisso stabile** (`**/skills/wiki-author/wiki-playbook.md` — i container differiscono per assistente, la coda no) + ricerca con `Glob`, guardia conservata e **migliorata** (ora riporta il glob cercato). **Corretta anche la causa a monte:** la nostra Definition of Done ordinava *«payload referenced by name»* — la regola generava il difetto; ora dice che **host-agnostico non è la stessa cosa di risolvibile**. Fermo dal 2026-07-11, immutato in 4 upgrade.

**✅ E10-FEAT-062 — il gate `wiki-guard` non è più cieco** *(su branch, non ancora consegnata)* · *dove:* requisiti EARS su `master` (merge `27e8472`/PR #251) + **specifica scritta** in `specs/124-copertura-changeset-scan/` (3 storie utente, 16 FR, 9 criteri; checklist 15/16) — **nessun codice ancora scritto** · SpecKit **completo fino a `implement`**: +32 test, gate reale verde (root 1423 · installer 595 · kit 207 · flow 149), **provato dal vivo via CLI** (registro → `pending 0` → lavoro ancora → `pending 1`) · *prossimo passo:* **consegna** · *decisione aperta:* **DA-1, la migrazione delle voci esistenti** (voce senza copertura = *copre tutto* → compatibile ma si porta dietro il buco · = *non copre nulla* → corretto ma al primo upgrade blocca ovunque) — **è una scelta utente, non di design**.
- **Il difetto:** `scan` azzera `pending` per la **sola presenza** della partizione di oggi fra le modifiche dell'albero (`scan.py:286-292`). Appena scrivi la voce di giornale il gate **smette di vedere tutto il lavoro successivo**: chi soddisfa la regola la disattiva, nella finestra esatta in cui lo `Stop` interroga. Il gate non richiede nemmeno una *voce* — un file **vuoto**, o una **riga vuota** appesa, lo soddisfano.
- **🔍 Misurato, non deduzione:** matrice di **15 scenari + 3 race** → **8 casi di non-rilevazione con una sola causa** · **1 difetto indipendente** (ogni invocazione git che fallisce degrada verso l'insieme vuoto **in silenzio**, continuando a dichiarare `anchor_kind: git` — e la nostra delega git **in background** rende quella race *normale*, non patologica) · **1 buco di perimetro** (`packages/` e `CLAUDE.md` fuori da `source_dirs`: il gate non guarda proprio la superficie che arriva agli ospiti → **E10-FEAT-063**).
- **Costo del gate, misurato:** ~**330 ms end-to-end a ogni turno**, 7 spawn di git. Il rimedio costa **+1 spawn** (~6%); l'identità di contenuto, che sembrava cara, è **un solo spawn per N path** (`git hash-object --stdin-paths`, verificato).
- **La classe è già nostra:** quinta istanza di [[identita-per-presenza-o-per-contenuto]], col segno peggiore — un via libera che *sembra* pulizia. Ed è **certificata da un test**: guardia e test condividono lo stesso errore di identità, per questo nessuno dei due l'ha vista.
- **Origine:** riscontro nodo *Acta* del 29/07 — *«un gate che non blocca mai è indistinguibile da un gate disinstallato»*, 8 nodi con la capability sul loro host. **Il verso dell'errore è comunque migliorato** rispetto al deadlock che la v0.3.2 ha chiuso: non si torna indietro su FEAT-045, la si completa.

**🚀 RILASCIATA v0.3.2 «il gate wiki riparato» — E10-FEAT-045** (contenuto: merge `4bfd090`/PR #239 · follow-up `4204bbc`/PR #240 · **release: tag `v0.3.2` su `3192167`/PR #241**, GitHub Release *latest* verificata via API, annuncio nel canale *Releases* della bacheca). L'ancora della rilevazione del lavoro non registrato è **derivata dalla storia**, non più stimata dagli orologi dei file.
- **Il difetto chiuso:** `scan` confrontava gli **mtime**; `gh pr merge` + `git pull` riscrivono i file mergiati **e il giornale insieme**, l'ordine diventa arbitrario → allo `Stop` il gate blocca, e per soddisfarlo bisogna scrivere ancora nel wiki, il che riproduce la condizione al merge dopo. **Una sessione non poteva chiudersi sul proprio ultimo merge**; l'unica uscita era l'albero sporco. Sette occorrenze in un giorno sul nodo *Acta*; da noi «funzionava», ma era **una corsa vinta, non correttezza**.
- **La forma del rimedio è il Principio XIV:** ancora **derivata** dall'ultima consegna che ha toccato il giornale + diff da lì, **unito alle modifiche non consegnate dell'albero di lavoro** (senza questa seconda metà il gate non vedrebbe **mai** il lavoro di una sessione in corso — il caso per cui esiste); **mtime dichiarato come proxy** con **causa tipizzata** dove l'ospite non è un repo, perché `scan` è host-agnostico *by design*. La macchina git **esisteva già a due file di distanza** (`ritual_check.py`) → riuso con estrazione.
- **🔍 La prova, misurata due volte.** A parità di stato del filesystem con l'mtime del giornale spostato indietro: vecchio `pending=14`, nuovo `pending=4` invariato. E **sul caso reale**, subito dopo questo stesso merge di 33 file + pull: `pending=1`, e quell'unico file è rumore locale — non i file mergiati. *Il difetto è stato chiuso dal proprio ultimo merge.*
- **Assorbe E10-FEAT-048** sugli host git: i file **gitignorati** non bloccano più la chiusura di sessione — non filtrati, **mai entrati** — e il blocco **nomina i file** invece di dirne il numero. Residuo non-git **dichiarato**, non simulato.
- **Il rischio presidiato:** lo schema `wiki.scan/1` **non si bumpa**. I consumatori lo confrontano per uguaglianza e vanno in **fail-open**, quindi un bump non romperebbe il gate: lo farebbe **sparire in silenzio** sugli ospiti non aggiornati. Guardia scritta **prima** dell'implementazione — e la compatibilità vale nelle **due direzioni** (il consumatore nuovo tollera il produttore vecchio, verificato dal vivo).
- **Un falso positivo trovato al PRIMO USO del gate appena consegnato, contro il suo autore:** contava come lavoro un file le cui uniche differenze erano i **fine-riga** (zero righe aggiunte, zero rimosse). `git status` risponde a «risulta modificato?», `git diff` a «il contenuto è cambiato?»: il gate pone la seconda domanda, avevo usato la prima. Distillato come **quarta istanza** di [[identita-per-presenza-o-per-contenuto]], a **segno invertito** (le tre esistenti producono un no-op che sembra successo, questa lavoro dichiarato dove non ce n'è).
- **Cinque difetti colti da test che fallivano, non da rilettura:** `git status` **collassa** le directory non tracciate · il campo `message` **non si tocca** (è la stringa localizzata dell'ospite) · `_hooklib.py` vive in **due copie** · il perimetro della guardia era **troppo largo** e falliva su un file corretto · un metodo del profilo citato a memoria.
- **Decisioni utente registrate:** «giudizio registrato» come terza forma di chiusura → **rinviato a E10-FEAT-051** (si misura in esercizio, invece di tarare una via d'uscita sul comportamento rotto) · **A-2**: una voce **non committata** vale, **ma solo quella di oggi** — il gate chiede se hai registrato, non se hai committato; una voce stantia viene **nominata**.

**🚀 RILASCIATA v0.3.3 «il lint che non grida al lupo»** (contenuto: merge `0ec1a4a`/PR #247 + `d82f127`/PR #248 · **tag `v0.3.3` su `d82f127`**). Un rilascio di **fiducia**: la guardia smette di segnalare il falso, e i blocchi distribuiti dicono finalmente *cosa fare* di ciò che una guardia riporta.
- **Il difetto chiuso:** `extract_wikilinks` leggeva `[[…]]` **dentro il codice** — un `[[requirement]]` in un esempio TOML, la forma del wikilink citata fra backtick — e **rompeva gli alias escapati in tabella** (`[[target\|alias]]`, forma Obsidian obbligatoria in una cella). **21 falsi positivi** su un wiki reale. Nessun gate si bloccava: il tooling è advisory — il danno è che *una guardia che segnala il falso insegna a smettere di leggerla*. E poiché l'estrattore è condiviso, quei link inquinati alimentavano anche i **candidati distill** del pavimento giornaliero.
- **🔍 Entrambi trovati USANDO lo strumento, non leggendone l'output** — il primo scrivendo una pagina, il secondo scrivendone un'altra un'ora dopo. Due difetti della stessa guardia in un'ora.
- **Le pratiche, che sono il vero contenuto:** la **regola del boy scout** (riparare nello stesso passaggio, coi tre vincoli: *è rotto davvero?* · *rimedio per classe* · *non risolvere nascondendo*) e **dove cominciare il lint semantico** — la deriva si concentra nelle pagine che **riassumono**, perché un riassunto è una copia la cui fonte continua a muoversi; base empirica **18 drift su 18**. Più due regole git nel blocco SDLC (*cosa manca davvero* davanti a un conflitto · *«modificato» ha due significati*).
- **Il buco che ha generato il pacchetto, colto dall'utente:** la regola del boy scout era finita nella **prosa dogfood** del `CLAUDE.md`, non nel blocco distribuito — quindi non esisteva per **nessun ospite**. È la regola 1 (*«se vive solo nel `CLAUDE.md` di Sertor è un prototipo, non una feature»*) violata **nel file che la dichiara**, e nessuna guardia confronta «pratica standing» con «pratica distribuita».
- **Budget dei blocchi alzati deliberatamente** (wiki 75→92, SDLC 70→74) col commento che nomina cosa li ha fatti crescere: alzarlo *è* il meccanismo della guardia, non un aggiramento.
- **Fuori dal pacchetto, per decisione:** l'audit del codice morto come skill (**E10-FEAT-061** — è una capacità da costruire) e i due principi di costruzione delle guardie, registrati **dentro FEAT-045** che li ha prodotti (disciplina di chi *scrive* una guardia, non pratica di chi la usa).
- **Corretto un EXEC che si contraddiceva:** FEAT-045 risultava **consegnata e insieme «da fare»** in due file, perché il numero copriva due cose. La divergenza `ritual-check`↔`wiki-guard` ha ora il suo numero, **E10-FEAT-060**.

**Nuova epica E16 `evoluzione-modello-wiki`** *(futura, non iniziata)* — l'handoff del nodo *Nunzio* promosso da idee a backlog: misurare quanto il core deterministico viene **aggirato** dall'LLM (dato-prima-del-codice) · backlink generati dal core · `plan` + coda dei «passivi» (**converge con E10-FEAT-049 e E13-FEAT-014**) · riproducibilità cross-OS in CI. *La tesi: i due estremi — `openwiki` tutto-LLM senza verifica, `wiki-compiler` tutto-codice — convergono da soli verso il centro dove siamo già; la richiesta non è spostare il confine D↔N, è sfruttarlo meglio.*

**Niente altro in corso.** **Rilasciata v0.3.1** «riparazione» — il comando d'upgrade non era eseguibile sugli host che **pinnano** il runtime (parse a mano del `pyproject.toml`: su una sorgente con `tag` il resto della tabella TOML finiva dentro la URL). Segnalato dal nodo *Acta* **poche ore dopo** la v0.3.0. Ora parse con `tomllib`, e il comando **sposta** il pin alla versione nuova invece di ripetere quella vecchia — o, per pin `rev`/`branch` non derivabili, lo **nomina** senza sbalzarne fuori l'ospite. **8 guardie che occupano configurazioni che il nostro nodo non ha.**
- **Perché non emergeva da noi, ed è la lezione:** il nostro runtime è **ref-less per progetto** (segue HEAD), quindi il ramo che rompe è **irraggiungibile** qui. Come lo formula *Acta*: **il difetto seleziona chi segue la disciplina** — solo chi pinna a un riferimento immutabile riceveva il comando rotto. Distillato come **terzo limite** in [[dogfood-fidelity]]: il dogfood è **una** configurazione, e **la più favorevole per costruzione**, perché la ripariamo a mano di continuo.
- **Altri 4 riscontri di Acta promossi a backlog:** `scan` conta i file **gitignorati** come lavoro non registrato (E10-FEAT-048 — **✅ assorbita da FEAT-045 sugli host git**, residuo non-git dichiarato) · il lint deve guardare i **riferimenti entranti** (E10-FEAT-049) · un emendamento si referenzia **per titolo, non per numero** (E13-FEAT-015) · il campo `**Version**` porta **due fatti** (E2-FEAT-026). Più E10-FEAT-045 rafforzata con la causa esatta.

**Rilasciata v0.3.0** «governance» (contenuto: merge `3b5dd6d`/PR #231 · release: tag su `45db7b0`/PR #232, GitHub Release *latest*, annuncio in bacheca): ratificato il **Principio XIV — «Derived State, Not Declared»** (costituzione dogfood **v1.6.0**, starter neutro **0.5.0**) e — per la prima volta — **gli emendamenti raggiungono gli ospiti che una costituzione ce l'hanno già**.
- **Il principio:** un fatto che vive in più posti va **derivato**, non copiato; dove derivarlo è impossibile serve un **riconciliatore nominato** che **dichiari** la divergenza. Conservare una copia stantia può essere giusto, conservarla **in silenzio** no. Ratificato da un **conteggio**, non da un'idea: cinque istanze in cinque domini in tre settimane, **quasi tutte scoperte da nodi a valle**. Nuovo **gate al `plan`**.
- **La sesta istanza, trovata mentre lo scrivevamo** (da una domanda dell'utente: «la costituzione come la stiamo rilasciando?»): `_apply_constitution` preservava una costituzione reale **in silenzio**, identico per install e upgrade → **un emendamento non arrivava mai** a chi ne aveva già una. Fix «dichiara, non scrivere»: si legge la versione dello starter **dentro** il file dell'ospite, si confronta, e se è indietro il report lo dice rimandando alla nuova sezione **`Amendments`** (che elenca cosa ha aggiunto *ogni* versione). Nulla viene scritto: integrare è giudizio dell'agente dell'ospite.
- **Prima istanza del XIV, dieci minuti dopo la ratifica: la nostra** — `wiki/log/index.md` duplica un fatto derivabile dalla cartella, senza riconciliatore, e il `lint` non lo vede (E10-FEAT-047).

**Rilasciata v0.2.1** «riparazione dal campo» (merge `3a3591e`/PR #228, tag + GitHub Release *latest*, annuncio e risposte su Acta). Il triage della bacheca — 5 segnalazioni da 4 nodi indipendenti — ha prodotto **9 righe di backlog**; i **due P0 sono consegnati e provati dal vivo attraverso il runtime installato**:
...
- **debito-tecnico (E10):** hook portabili/ancorati · fail-loud breadcrumb · `distill-floor` merge-gate (v0.1.3) · `wiki-guard` Stop-gate (v0.1.4) · fix upgrade double-wire Stop (FEAT-041, v0.1.5) · **ancora derivata del gate wiki (FEAT-045, assorbe FEAT-048)** — *residuo: FEAT-042 (dry-run non proietta i settings-merge)*
...
*Stato per epica, una riga:* E1 ✅ · E2 🔄 · E3 🔄 · E4 🔄 (quasi) · E5 🔄 · E6–E9 📋 · E10 🔄 · E11 differita · E12 🔄 · E13 🔄 · E14 🔄 · E15 🔄 · **E16 📋 futura**.

### 📋 Prossime direzioni (da scegliere)
- **Nuove dal 2026-07-27 — audit del codice morto** (merge `3fcabf2`/PR #244): **E10-FEAT-053** (3 shim + i loro 13 test) · **E10-FEAT-054** *(Should — `specaudit/observability.py` non è referenziato da nulla **e il suo docstring dichiara di realizzare l'osservabilità costituzionale**: il difetto è l'affermazione, non le 27 righe)* · **E10-FEAT-055** (3 formattatori «regression» superati) · **E10-FEAT-056** (13 simboli vivi solo nei test) · **E10-FEAT-057** (`migrate`: obsoleto, non morto) · **E10-FEAT-052** (cache embedding in `float64` dove lo store usa `float32`: ~200 MB su ogni host — PR #243 aperta). Metodo distillato in [[audit-codice-morto]].
- **🎯 E10-FEAT-049 è PRONTA da chiudere:** il lint semantico deve guardare i **riferimenti entranti**. Il lint del 27/07 le ha dato **quattro casi di prova reali** (`ports-adapters` «otto»/indice «sei» · `constitution` «14 principi»/indice «10» · `speclift` «8 stadi»/indice «9» · pannello «fatto»/indice «in sviluppo») e **due rilevatori già in prototipo** (~40 righe): indice ↔ pagina puntata, e `sources:` ↔ data dell'ultimo commit. *Nota di scope: è la stessa forma di **E13-FEAT-014** (anti-drift della doc utente) — conviene progettarle insieme.*
- **Decisione sospesa: speclift + specaudit** — 3.916 righe di codice e 3.442 di test (**13%** del prodotto), **non distribuiti a nessun ospite**, mai usati. Usare · congelare · ritirare: il limbo costa manutenzione silenziosa.
- **Coda della riparazione** (aperta dal triage 2026-07-25, dopo i due P0): E2-FEAT-023 (`upgrade` nudo non copre le capability, summary verde ingannevole) · E2-FEAT-024 (minori: `__pycache__`, lingua del config) · E10-FEAT-043 (breadcrumb mai ripulito) · E10-FEAT-044 (lo skew spegne in silenzio gli aiuti advisory) · **E10-FEAT-060** (`ritual-check` vs `wiki-guard` misurano realtà diverse — *era numerata 045, numero già consegnato; riscontrata dal vivo il 28/07*) · E10-FEAT-046 (`upsert-index` sezione+wikilink) · E12-FEAT-014 (prima query >2 min senza segnale) · **E13-FEAT-014 (guardia deterministica anti-drift della doc utente — la richiesta di fondo di *Acta*)**.
...
- **🆕 E4-FEAT-015 — continuità di sessione (⚠️ DA ELICITARE)** *(richiesta utente 2026-07-29)*: al riavvio iniettare nel contesto gli **ultimi ~2 turni** della conversazione precedente. L'archivio conserva già tutto ma si consulta **a domanda**: manca la superficie di **restituzione all'avvio**. Il caso più frequente della memoria non è «ne avevamo parlato mesi fa» ma «cosa stavamo facendo cinque minuti fa» — l'unico che oggi non serviamo. **Non è ancora un requisito:** da decidere quali turni, quale sessione, budget di contesto, gate privacy (l'iniezione automatica è più invasiva della ricerca a domanda), *quando* iniettare, e se iniettare o **istruire** il flusso a caricarli (il canale-hook è limitato a ~10k caratteri).
- **🆕 E15-FEAT-011 — il terzo asse della fedeltà** *(scritto il 2026-07-29 dopo due rinvii)*: nessuna guardia confronta *ciò che facciamo* con *ciò che spediamo*. Nel caso reale (regola del boy scout) asset-fidelity e process-fidelity erano **entrambe perfette** e il difetto è passato lo stesso. Il rilevatore è difficile per costruzione → [[pratica-standing-vs-pratica-distribuita]].
...
### 📋 Le 16 epiche (per stato) — stato al 2026-07-28
...
| **E5** | 🆕 [`retrieval-qualita`](../../requirements/retrieval-qualita/epic.md) | 🔄 FEAT-001+011 ✅ · FEAT-003 T1+T2 ✅ su master | **eval IR ✅** (PR #92) + **graph-eval ✅** (FEAT-011) + skill live ✅ + **FEAT-003 misura fusione + `search_combined` strutturato (tupla, metrica OR) ✅** (merge `42aceaf`+`908bd92`). **Scoperta:** lo «0.17» era artefatto dell'AND; a OR union=1.00, il debole misurato allora era **`search_docs`** — *quel MRR 0.55 era il numero di giugno: superato dal dedup fuzzy, il pavimento oggi è **hit@5 0.88 / hit@3 0.625** col gate PASS. E la superficie di prodotto è `search_combined` a tre flussi, non `search_docs` isolato*. Restano FEAT-002/004/005-007 |
...
| **E16** | 🆕 [`evoluzione-modello-wiki`](../../requirements/evoluzione-modello-wiki/epic.md) | 📋 **futura — non iniziata** | **Sfruttare meglio il confine D↔N del sistema-wiki**, senza spostarlo. Origine: handoff del nodo *Nunzio* (2026-07-09) da un dogfooding reale + analisi sul codice di `openwiki` e `wiki-compiler` — *i due estremi convergono da soli verso il centro dove Sertor è già*. Backlog: **FEAT-001** misurare quanto il core deterministico viene **aggirato** dall'LLM (dato-prima-del-codice; il solo campione noto è *zero invocazioni di `upsert-index`* su un progetto ben istruito) · **FEAT-002** backlink generati dal core (relazione inversa = aritmetica, non giudizio) con rewriter section-aware · **FEAT-003** `sertor-wiki-tools plan` + coda dei «passivi» — **converge con E10-FEAT-049 e E13-FEAT-014, progettare insieme** · **FEAT-004** riproducibilità cross-OS in CI + benchmark `lint` (sospetto O(n²) sugli orfani). **Fuori ambito:** connettori d'ingestione → **E7**; comunicare il differenziatore → **E13 Fase 2**. **Già chiuso, non riaprire:** il bug dell'hook `SessionStart` (percorsi hard-coded) — risolto da E10-FEAT-029 + migrazione `.ps1`→`.py` |
*Legenda:* ✅ completa · 🔄 nucleo consegnato, residui aperti · 📋 da fare · 🆕 nuova. *Numerazione `E1`..`E15`: vista standing per epica (E1 nucleo `sertor-core`, E11 `multiutente` differita, E12 `usabilità`, E13 `documentazione-marketing`, E14 `speclift`, E15 `fedelta-dogfood` nuova 2026-07-03 da audit fedeltà, E16 `evoluzione-modello-wiki` nuova 2026-07-28 dall'handoff Nunzio); E1–E4 storiche, E5–E10 dal backlog audit 2026-06-16, E12 dall'esplorazione UX 2026-06-23, E13 dalla richiesta 2026-06-24, E15 dalla domanda «abbiamo fatto su tutto Sertor?».*
...
| **→ Evoluzione del modello wiki (4 idee)** | Le richieste dell'handoff *Nunzio* — core D↔N aggirato · backlink deterministici · `plan` + coda dei passivi · riproducibilità cross-OS — **sono uscite da qui**: sono state promosse a **[epica E16 `evoluzione-modello-wiki`](../../requirements/evoluzione-modello-wiki/epic.md)** (FEAT-001..004) | Regola della sezione: un'idea promossa a epica vive nel backlog dell'epica, **non qui** (A-12, no duplicazione di stato) | ✅ **promosse a E16 (2026-07-28)** |
