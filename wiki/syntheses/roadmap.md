...
- **Prossimo passo aperto — CODA (decisioni utente 2026-07-18, analisi [[setup-dichiara-presunzione-non-azione]]):**
  **✅ (1) E10-FEAT-038 `doctor` ancorato alla root — CONSEGNATA** (merge `7075a0f`/PR #198, 2026-07-18): `Path.cwd()` →
  `settings.project_root` self-localizing (parent di `.sertor/` via `sys.prefix`; `CLAUDE_PROJECT_DIR` override; fail-loud se
  irrisolvibile). **Invarianza al cwd provata LIVE** sul runtime installato (stesso verdetto da root e da `src/`) + 5 unit test.
  → **PROSSIMO: (2) E10-FEAT-033** (`ritual-check` rileva il default branch) → (3) E2-FEAT-018 allargata (esito-descrive-azione
  + log `.install-log.jsonl`; **E10-FEAT-036 folded**) → (4) E10-FEAT-034 (`rag-freshness` rimisura, ora **sbloccata** dal doctor
  ancorato). *Tesi unificante:* «il setup dichiara ciò che presume, non ciò che è successo» (Principio XII).
  ...
  «prossimo passo» finché l'utente non lo riapre.** (3) 🐛 **E4 cattura-auto memoria — verificare se si è chiusa da sé**: l'ipotesi
