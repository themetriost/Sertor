...
<!-- EXEC:START -->
## ⚡ Executive summary (stato al 2026-07-28)
...
**Consegnata l'igiene del 28/07** (merge `0ec1a4a`/PR #247, **su `master`, non rilasciata** — policy A-15: `/VERSION` si bumpa solo a una release user-facing). Due fix di prodotto a `sertor_core.wiki_tools` con test: l'estrattore di wikilink **leggeva dentro i blocchi di codice** (un `[[requirement]]` in un esempio TOML e i `` `[[x]]` `` fra backtick risultavano link rotti) e **rompeva gli alias escapati in tabella** (`[[target\|alias]]`, forma Obsidian) — 21 falsi positivi in tutto, entrambi trovati **usando** lo strumento. Più: `CLAUDE.md` **−92 righe** di contenuto derivabile (~1.570 token a ogni sessione) e **4 istruzioni corrette** che insegnavano `RAG_BACKEND`, manopola che `Settings.load` **ignora** da FEAT-011; **lint strutturale a 0 su tutte e cinque le chiavi** (40 segnalazioni accumulate, riparate *per classe*); l'archivio `processed/` **catalogato** invece che nascosto ([[archivio-processati]]) dopo una correzione dell'utente — *archiviato non vuol dire nascosto*; **regola del boy scout** standing nel rituale (+E10-FEAT-059 per la metà deterministica).
