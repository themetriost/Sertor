...
| **E12** | 🆕 [`usabilità`](../../requirements/usabilita/epic.md) | 🔄 MVP avviato (FEAT-001 doctor ✅ su branch `074`) | **owner del layer UX** (skill agentiche + agente *concierge* + poche primitive deterministiche, D↔N). **FEAT-001 `sertor-rag doctor` implementata** (SpecKit completo, su branch `074-doctor-salute`, in attesa di merge). Resta MVP: **FEAT-002 guided-setup** (Must, decomposta). Assorbe gli item UX-facing da E2/E3/E10 (i *meccanismi* restano lì, cross-ref) |
...
> **Attiva su branch `074-doctor-salute`: E12-FEAT-001 `sertor-rag doctor` ✅** — SpecKit completo
> specify→plan→tasks→implement, Constitution 12/12 + missione, root 1118 verdi / sertor 333 verdi,
> ruff clean. *Cosa:* primitiva deterministica «ha funzionato?» (4 aree env/provider/indice/MCP,
> esito pass/warn/fail + causa/rimedio, `--json` schema `doctor.report/1`, exit-code gate, offline-safe
> con probe provider opt-in `--online`); chiude il `--check` *deferred* di E2/FEAT-003. *Dove:* branch
> `074`, commit `612ea4f`. *Prossimo passo concreto:* **merge su `master`** → re-index dogfood + smoke MCP.
> *Blocco/decisione aperta:* nessuno (DA-D4/D5 risolte; debito stantio-MCP forte già promosso).

**Candidati a valore = Should aperti:**

- **E12-FEAT-002 → guided-setup (Must, `usabilità`)** — il secondo MVP dell'epica: skill (+ ramo
  concierge) che conduce install→configure→verify conversando e chiude con `doctor`. Decomposta in
  requisiti EARS; ora che `doctor` esiste, la skill non è più cieca. *Primo passo:* `/speckit-specify`.
