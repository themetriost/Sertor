...
### 🔄 In progress
- **🔴 PRIORITÀ — Installer rotto sugli ospiti (4 segnalazioni federazione, 2026-07-24).** *Cosa:* quattro nodi (*VM-WorkingFolder*, *Sinthari*, *Kaelen*, *Acta*) segnalano indipendentemente che **l'upgrade dichiara successo senza aggiornare il runtime**: su Sinthari il pin git è fermo a **0.1.0 da tre upgrade e un mese**; il comando pubblicato dall'auto-updater (`uvx --refresh sertor`) **fallisce**; una `.mcp.json` con `--directory` ha reso un RAG **cieco per un mese** e l'installer la salta perché «già registrata». *Dove:* bacheca Acta, canale `Feedback Sertor`; analisi in [[esito-sull-host-vs-forma-dell-asset]]. *Prossimo passo:* 3 fix — comando in `version-check-start.py:48` · `uv lock --upgrade-package sertor-core` nell'upgrade · l'upgrade riporta la **versione effettiva letta dal runtime**. *Blocco:* **nessun bump/release finché non sono fatti** — una 0.2.0 oggi non arriverebbe a nessuno.
- **E5-FEAT-012 — Contratto di retrieval verso l'agente (fan-out del grafo).** *Cosa:* ✅ **il code-graph è dentro `search_combined`** come terzo flusso etichettato, `SERTOR_COMBINED_GRAPH` **acceso di default** (decisione utente 2026-07-24, che supera FR-019). Assenza tipizzata a 3 cause, troncamento dichiarato, provenienza degli ingressi, corroborazione bilaterale. *Dove:* branch `118-contratto-retrieval-agente`, 8 commit; design note [[llm-facing-retrieval-contract]] (4 giri di revisione esterna). *Prossimo passo:* **merge**, che richiede suite verde. *Blocco:* **4 test rossi pre-esistenti** in `test_observability_capture.py` (accoppiamento d'ordine, indipendenti da questa feature) — il gate pre-merge resta chiuso.
  **Scoperta abilitante:** la macchina di valutazione esistente misura il *retrieval*, non il *comportamento dell'agente*. Il gate di misura è stato **promosso a feature propria** (E5-FEAT-013, harness agent-facing) e la capacità è consegnata **senza attenderlo**: rischio accettato e dichiarato in `spec.md`.
