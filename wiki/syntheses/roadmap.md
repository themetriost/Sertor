...
### 🔄 In progress
**🔨 PRONTA AL MERGE — E10-FEAT-045: il gate `wiki-guard` non è più insoddisfacibile.** *Dove:* branch `123-feat-045-ancora-derivata-scan`, **SpecKit completo e implementazione chiusa** (spec+clarify · plan Constitution 14/14 · 40 task · fasi 1-7). *Prossimo passo concreto:* **PR aperta, in attesa del go dell'utente per il merge**; a valle, catena post-merge (re-lock runtime → re-index → smoke) che chiude anche il ciclo hook-nuovo + runtime-nuovo.
- **Il difetto:** `scan` ancora al **mtime** del log (`scan.py:41-58`); `gh pr merge` + `git pull` riscrivono i file mergiati **e il giornale insieme**, l'ordine diventa arbitrario → allo `Stop` il gate blocca, e per soddisfarlo bisogna scrivere ancora nel wiki, il che riproduce la condizione al merge dopo. **Una sessione non può chiudersi sul proprio ultimo merge**; l'unica uscita è l'albero sporco. Sette occorrenze in un giorno sul nodo *Acta*. **Il nostro `pending=0` è una corsa vinta, non correttezza.**
- **Forma del rimedio** (Principio XIV): ancora **derivata dal diff git** dove l'ospite è un repo, **mtime dichiarato come proxy** dove non lo è (`scan` è host-agnostico *by design*, `scan.py:4`). La macchina git **esiste già a due file di distanza** (`ritual_check.py`: `_git`, `_resolve_base`, `_changed_repo_paths`, con default-branch a runtime da E10-FEAT-033) → **riuso con estrazione**, non nuova infrastruttura.
- **Assorbe E10-FEAT-048** sugli host git: il diff esclude i file **gitignorati** per costruzione e fornisce **i nomi** dei file da mettere nel messaggio di blocco (oggi dice *quanti*, non *quali*).
- **Due cose emerse scrivendo la spec, non presenti nel backlog:** (a) il calcolo deve includere **anche l'albero di lavoro non committato**, altrimenti il gate non vedrebbe **mai** il lavoro di una sessione in corso — che è il momento in cui interviene; (b) **l'identificativo di schema `wiki.scan/1` NON si può bumpare**: i due hook consumatori fanno `schema != "wiki.scan/1"` → **fail-open**, quindi un bump non romperebbe il gate, lo farebbe **sparire in silenzio** su ogni ospite non aggiornato. Da coprire con una guardia.
- **✅ Consegnato sul branch, con la prova misurata:** a parità di stato del filesystem (mtime del giornale spostato indietro) il codice **vecchio** dice `pending=14`, il **nuovo** `pending=4` invariato. Gate: **1376 test**, ruff pulito; `test_wiki_tools_scan.py` passa **senza essere toccato**. Consegna host-facing completa: i due hook **nominano i file**, doc utente aggiornata (`troubleshooting` + `reference` — che non documentava affatto `sertor-wiki-tools`).
- **Tre difetti colti da test che fallivano, non da rilettura:** `git status` **collassa** le directory non tracciate (senza `-uall` avrebbe nominato `src/` invece dei file) · il campo `message` **non si tocca** — è la stringa localizzata dell'ospite, asserita per uguaglianza esatta · `_hooklib.py` vive in **due copie** e ne avevo allineata una sola (colto dalla guardia di sync: duplicazione *presidiata*, non silenziosa).
- **✅ Bivio del «giudizio registrato» RISOLTO (decisione utente 2026-07-27): rinviato**, promosso a **E10-FEAT-051** con il motivo scritto — con l'ancora corretta i sintomi peggiori spariscono da soli, quindi il residuo legittimo si misura **in esercizio** invece di tararci sopra una via d'uscita adesso, sul comportamento rotto.
- **✅ A-2 decisa in `clarify`:** una voce **scritta ma non committata** vale come registrazione — necessario, perché allo `Stop` niente è ancora committato e pretendere un commit sarebbe un deadlock nuovo — **ma solo se è quella di oggi**. Senza scadenza, un giornale dimenticato spegnerebbe il gate per giorni, rendendo *legittima* la via d'uscita dell'albero sporco invece di toglierla. Una voce stantia viene **nominata**, non ignorata in silenzio.

Niente altro attivo. **Rilasciata v0.3.1** «riparazione» — il comando d'upgrade non era eseguibile sugli host che **pinnano** il runtime (parse a mano del `pyproject.toml`: su una sorgente con `tag` il resto della tabella TOML finiva dentro la URL). Segnalato dal nodo *Acta* **poche ore dopo** la v0.3.0. Ora parse con `tomllib`, e il comando **sposta** il pin alla versione nuova invece di ripetere quella vecchia — o, per pin `rev`/`branch` non derivabili, lo **nomina** senza sbalzarne fuori l'ospite. **8 guardie che occupano configurazioni che il nostro nodo non ha.**
...
- **Altri 4 riscontri di Acta promossi a backlog:** `scan` conta i file **gitignorati** come lavoro non registrato (E10-FEAT-048, blocca la chiusura di sessione su ogni host) · il lint deve guardare i **riferimenti entranti** (E10-FEAT-049) · un emendamento si referenzia **per titolo, non per numero** (E13-FEAT-015) · il campo `**Version**` porta **due fatti** (E2-FEAT-026). Più E10-FEAT-045 rafforzata con la causa esatta.
